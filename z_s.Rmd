...
difference_data <- data.frame(
  Dataset = rep(c("WP_CON", "WP_UNCON",  "GHSL", "GPW_UN"), each = nrow(camps)),
  Population_Difference = c(as.numeric(camps$wp_c_df), as.numeric(camps$wp_uc_df), as.numeric(camps$ghsl_df),as.numeric(camps$gpwU_df))
)
...
boxplot_st_pp_d <- ggplot(camp_final, aes(x = dou_level1, y = st_pp_d, fill = dou_level1)) +
  ...
  labs(title = "Distribution of st_pp_d by dou_level1",
       x = "Dou Level 1",
       y = "st_pp_d",
       fill = "Dou Level 1") +
  theme_minimal() +
...
difference_data_dou <- data.frame(
  Dataset = rep(c("WP_CON", "WP_UNCON", "GHSL", "GPW_UN"), each = nrow(camp_final)),
  Population_Difference_dou = c(as.numeric(camp_final$wp_c_df), as.numeric(camp_final$wp_uc_df), as.numeric(camp_final$ghsl_df),as.numeric(camp_final$gpwU_df)),
...
#building box focus and flipping negative coord
...
boxplot_ratio <- ggplot(difference_data_dou, aes(x = Dataset, y = -ratio, fill = dou_level1)) +
  ...
  stat_summary(fun = mean, geom = "point", shape = 23, size = 0.3, color = "blue") +  # Changing the color of the mean
  stat_summary(fun.data = mean_cl_normal, geom = "text", 
               ...
               vjust = -0.5, size = 2, color = "black") +  # Changing the color of the mean
  labs(title = "Ratio of Difference to Survey Population by EO Population Data Products and dou_level",
       x = "Dataset", y = "Ratio of Difference to Survey Population", fill = "dou_level") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
