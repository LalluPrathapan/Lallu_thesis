...
#Packages install of exactextracter 
```{r}
...
camps$w_c<-as.numeric(camps$w_c)
camps$wp_c<-as.numeric(camps$w_c)
camps$wp_uc<-as.numeric(camps$w_uc)
...
#Subsetting the camps less than 10000 square meter area
...
aggregated_data <- camps %>%
  mutate(
    AreaCategory = case_when(
      cl_Shp_a > 10000 ~ "> 10,000",
      cl_Shp_a <= 10000 ~ "< 10,000"
    ),
    Category = paste(Sit_Stt, "(area", AreaCategory, ")")
  ) %>%
  group_by(Category, Sit_Stt) %>%
  summarise(Count = n(), .groups = 'drop')

# Create combined bar plot with counts on the bars
bar_plot_combined <- ggplot(aggregated_data, aes(x = Category, y = Count, fill = Sit_Stt)) +
  geom_bar(stat = "identity", alpha = 0.8) +
  geom_text(aes(label = Count), vjust = -0.5, size = 4) + # Add count labels above bars
  labs(
    title = "Count of Camps by Type and Area",
    x = "Camp Type and Area",
    y = "Count",
    fill = "Camp Status"
  ) +
...
formal_large <- camps %>% filter(Sit_Stt == "Formal" & N_INDVI > 10000) %>% mutate(Type = "Formal (N_INDVI > 10000)")
formal_small <- camps %>% filter(Sit_Stt == "Formal" & N_INDVI < 10000) %>% mutate(Type = "Formal (N_INDVI < 10000)")
informal_large <- camps %>% filter(Sit_Stt == "Informal" & N_INDVI > 10000) %>% mutate(Type = "Informal (N_INDVI > 10000)")
informal_small <- camps %>% filter(Sit_Stt == "Informal" & N_INDVI < 10000) %>% mutate(Type = "Informal (N_INDVI < 10000)")

# Combine datasets
combined_data <- rbind(formal_large, formal_small, informal_large, informal_small)

# Aggregate counts for each category
aggregated_data <- combined_data %>%
  group_by(Type, Sit_Stt) %>%
  summarise(Count = n(), .groups = "drop")

# Create a bar plot with counts on bars
bar_plot_combined_f_if_plg <- ggplot(aggregated_data, aes(x = Type, y = Count, fill = Sit_Stt)) +
  geom_bar(stat = "identity", alpha = 0.8) +                # Bar plot
  geom_text(aes(label = Count), vjust = -0.5, size = 4) +   # Count labels above bars
  labs(
    title = "Count of Camps by Type and N_INDVI",
    x = "Camp Type and N_INDVI",
    y = "Count",
    fill = "Camp Status"
  ) +
...
```
#Do not run, the dataset provided already has extracted value
#______________________________________________
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
extract_sum  <-  function(raster, polygon, field){
  ...
  x[,"w_c"] <- x[, "value"] * x$coverage_fraction

  x_sum <- x %>%

    dplyr::group_by(id_tmp)%>%

    dplyr::summarise_at("w_c", sum, na.rm = TRUE)
...
camps$ghsl_difference <-camps$N_INDVID - camps$ghsl
...
# Subtract N_INDVID from ras_name
#!!!!!! careful different dataset
camps$wp_uc_df <- as.numeric(camps$N_INDVI) - as.numeric(camps$w_uc)
...
# Calculate percentage difference
percentage_difference_wp_uc <- abs((camps$w_uc - camps$N_INDVI) / camps$N_INDVI) * 100

# Calculate root mean square error (RMSE)
rmse_wp_uc<- sqrt(mean((camps$wp_uc - camps$N_INDVI)^2))

# Calculate mean absolute percentage error (MAPE)
mape_uc <- mean(percentage_difference_wp_uc, na.rm = TRUE)
...
camps_wp_uc <- extract_sum(World_pop_constr,camps,"Name")
...
#for camps dataset correcting differences
```{r}
camps$wp_c_df<- as.numeric(camps$N_INDVI) - as.numeric(camps$wp_c)
print(camps$wp_c_df)
print(camps$wp_uc_df)
```
#%%%%%%%%%WORDPOP CONST END%%%%%%%%%%
...
#___WORLD_POP_C END________________________
#____________________________________________
#A.Correlation between houshold and population(both survey and estimated WP_UC)
...
# Calculate absolute difference
absolute_difference_wp_c <- abs(camps$w_c - camps$N_INDVI)

# Calculate percentage difference
percentage_difference_wp_c <- abs((camps$w_c - camps$N_INDVI) / camps$N_INDVI) * 100

# Calculate root mean square error (RMSE)
rmse_w_c <- sqrt(mean((camps$w_c - camps$N_INDVI)^2))
...
# Calculate absolute error
absolute_error_wp_c <- mean(abs(as.numeric(camps$w_c) - as.numeric(camps$N_INDVI)))

# Print the result
print(absolute_error_wp_c)

# Calculate percentage error
percentage_error_wp_c <- mean(abs(as.numeric(camps$w_c) - as.numeric(camps$N_INDVI)) / as.numeric(camps$N_INDVI)) * 100
...
```{r}
# Resample the 1 km resolution raster to 100 m resolution (using bilinear interpolation)
gpw_resampled <-  rast('D:/EAGLE/Master_Thesis_Hannes/1/1/thesisused-gpw.tif')

# Now you can compare this resampled GPW raster with the WorldPop raster
```
...
extract_sum  <-  function(raster, polygon, field){
  ...
  x[,"gpw_UN_re"] <- x[, "value"] * x$coverage_fraction

  x_sum <- x %>%

    dplyr::group_by(id_tmp)%>%

    dplyr::summarise_at("gpw_UN_re", sum, na.rm = TRUE)
...
```{r}
camps <- extract_sum(gpw_resampled, camps,"Name")
```
...
combined_plot1 <- grid.arrange(scatter_gpw_UN, scatter_wp_con, scatter_wp_uncon, scatter_ghsl, nrow = 2, ncol = 2)
...
min_value <- min(c(min( camps$wp_c, camps$wp_uc, camps$ghsl, camps$gpw_UN)))
max_value <- max(c(max( camps$wp_c, camps$wp_uc, camps$ghsl, camps$gpw_UN)))
...
create_scatter_plot <- function(data, x_var) {
  ggplot(data, aes_string(x = x_var, y = "N_INDVI", color = "Sit_Stt")) +
    ...
    coord_fixed(ratio = 1) + coord_flip() # Set equal aspect ratio
}
...
combined_plot_scale_adj <- grid.arrange( scatter_wp_con, scatter_wp_uncon, scatter_ghsl, scatter_gpw_UN,
                               nrow = 2, ncol = 2)
...
# Arrange the plots using grid.arrange
combined_smooth <- grid.arrange(scatter_gpw_UN,  scatter_ghsl, scatter_wp_con, scatter_wp_uncon,
                              nrow = 2, ncol = 2)
...
# Arrange the plots using grid.arrange
combined_plot_smooth_line <- grid.arrange( scatter_gpw_UN,scatter_ghsl,scatter_wp_con, scatter_wp_uncon,  
                              nrow = 2, ncol = 2)
...
# Find the minimum and maximum values of the estimated population variable across all datasets
min_value <- min(c(camps$wp_c, camps$wp_uc, camps$ghsl, camps$gpw_UN), na.rm = TRUE)
max_value <- max(c(camps$wp_c, camps$wp_uc, camps$ghsl, camps$gpw_UN), na.rm = TRUE)
...
# Scatter plot for GPW
scatter_gpw <- create_scatter_plot_with_fit(camps, "gpw_UN", "GPW ", min_value, max_value)

# Scatter plot for GHS-POP
scatter_ghsl <- create_scatter_plot_with_fit(camps, "ghsl", "GHS-POP ", min_value, max_value)

# Scatter plot for WorldPop Unconstrained
scatter_wp_uncon <- create_scatter_plot_with_fit(camps, "wp_uc", "WorldPop Unconstrained", min_value, max_value)

# Scatter plot for WorldPop Constrained
scatter_wp_con <- create_scatter_plot_with_fit(camps, "wp_c", "WorldPop Constrained", min_value, max_value)

# Arrange the plots in the specified order using grid.arrange
combined_plot_smooth_line <- grid.arrange(
  scatter_gpw, scatter_ghsl,
  scatter_wp_uncon, scatter_wp_con,
  nrow = 2, ncol = 2
)
...
# Filter for formal camps only
formal_camps <- camps[camps$St_Stt == "formal", ]

# Find the minimum and maximum values of the estimated population variable across all datasets
min_value <- min(c(formal_camps$wp_c, formal_camps$wp_uc, formal_camps$ghsl, formal_camps$gpw_UN), na.rm = TRUE)
max_value <- max(c(formal_camps$wp_c, formal_camps$wp_uc, formal_camps$ghsl, formal_camps$gpw_UN), na.rm = TRUE)

# Function to create scatter plot with fit line and consistent x-axis scale
create_scatter_plot_with_fit <- function(data, x_var, title, min_x, max_x, line_color) {
  ggplot(data, aes_string(x = x_var, y = "N_INDVI")) +
    geom_jitter(aes(y = as.numeric(N_INDVI), x = as.numeric(get(x_var))),
                size = 0.6, alpha = 0.8, color = "darkgray") +  # Use fixed jitter color
    geom_smooth(method = "lm", se = FALSE, color = line_color) +  # Add fit line with specified color
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +  # Add diagonal line
    labs(title = title,
         x = "Estimated Population",
         y = "Survey Population") +
...
# Scatter plot for GPW
scatter_gpw <- create_scatter_plot_with_fit(formal_camps, "gpw_UN", "GPW", min_value, max_value, "blue")

# Scatter plot for GHS-POP
scatter_ghsl <- create_scatter_plot_with_fit(formal_camps, "ghsl", "GHS-POP", min_value, max_value, "green")

# Scatter plot for WorldPop Unconstrained
scatter_wp_uncon <- create_scatter_plot_with_fit(formal_camps, "wp_uc", "WorldPop Unconstrained", min_value, max_value, "red")

# Scatter plot for WorldPop Constrained
scatter_wp_con <- create_scatter_plot_with_fit(formal_camps, "wp_c", "WorldPop Constrained", min_value, max_value, "purple")
...
# Filter for formal camps only
formal_camps <- camps[camps$Sit_Stt == "Formal", ]

# Combine the datasets into a long format for easier plotting
library(tidyr)

formal_camps_long <- formal_camps %>%
  pivot_longer(
    cols = c("gpw_UN", "ghsl","wp_uc","wp_c",),
    names_to = "Dataset",
    values_to = "Estimated_Population"
  )

# Create a scatterplot with different colored lines for each dataset
form_scatterplot<-ggplot(formal_camps_long, aes(y = as.numeric(Estimated_Population), x = as.numeric(N_INDVI), color = Dataset)) +
  geom_jitter(size = 0.6, alpha = 0.8) +  # Scatter points
  geom_smooth(method = "lm", se = FALSE) +  # Add fit lines for each dataset
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +  # Diagonal line
  labs(
    title = "Comparison of Population Estimates for Formal Camps",
    y = "Estimated Population",
    x = "Survey Population",
    color = "Dataset"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
print(form_scatterplot)
ggsave("Plots/form_scatterplot.png", plot = form_scatterplot, width = 8, height = 6, dpi = 400)

```

#_thes Formal scatterplot for all dataset in one
```{r}
...
formal_camps_long <- formal_camps %>%
  ...
  mutate(
    Dataset = recode(Dataset,
                     "gpw_UN" = "GPW-V4",
                     "ghsl" = "GHS-POP",
                     "wp_uc" = "WorldPop Unconstrained",
                     "wp_c" = "WorldPop Constrained")
  )

# Define colorblind-friendly palette for easy visualization
dataset_colors <- c(
  "GPW-V4" = "#e9724d",               # pink
  "GHS-POP" = "#d6d727",              # turquise
  "WorldPop Unconstrained" = "#92cad1", # majenta
  "WorldPop Constrained" = "#868686"   #skybleu Orange
)

# Create a scatterplot with improved aesthetics
form_scatterplot <- ggplot(formal_camps_long, 
                           aes(x = as.numeric(N_INDVI), 
                               y = as.numeric(Estimated_Population), 
                               color = Dataset)) +
  geom_jitter(size = 0.8, alpha = 0.7) +  # Scatter points with jitter
  geom_smooth(method = "lm", se = FALSE) +  # Fit lines for each dataset
  geom_abline(intercept = 0, slope = 1, 
              linetype = "dashed", color = "black", linewidth = 0.7) +  # 1:1 reference line
  scale_color_manual(values = dataset_colors) +  # Apply custom colors
  labs(
    title = "Comparison of Population Estimates for Formal Camps",
    x = "Survey Population",
    y = "Estimated Population",
...
ggsave("Plots/form_scatterplot.png", plot = form_scatterplot, 
       width = 8, height = 6, dpi = 400)

```
#formal scatterplot with rsquared
```{r}
# Calculate R² values for each dataset
r_squared_values <- formal_camps_long %>%
  group_by(Dataset) %>%
  summarise(
    r_squared = cor(as.numeric(N_INDVI), as.numeric(Estimated_Population), use = "complete.obs")^2
  )
...
#_formal scat<10000pop all inone
```{r}
library(ggplot2)
library(tidyr)

# Filter for formal camps with N_INDVI < 10,000
formal_camps_ls10000 <- camps[camps$Sit_Stt == "Formal" & camps$N_INDVI < 10000, ]

formal_camps_ls10000_long <- formal_camps_ls10000  %>%
  pivot_longer(
    cols = c("gpw_UN", "ghsl", "wp_uc", "wp_c"),
    names_to = "Dataset",
    values_to = "Estimated_Population"
...
# Create a scatterplot with improved aesthetics
form_scatterplot_ls10000 <- ggplot(formal_camps_ls10000_long, 
                           aes(x = as.numeric(N_INDVI), 
                               y = as.numeric(Estimated_Population), 
                               color = Dataset)) +
  geom_jitter(size = 0.8, alpha = 0.7) +  # Scatter points with jitter
  ...
  scale_color_manual(values = dataset_colors) +  # Apply custom colors
  labs(
    title = "Comparison of Population Estimates for Formal Camps with population<10000",
    x = "Survey Population",
    y = "Estimated Population",
    color = "Dataset"
  ) +
...
# Print the plot
print(form_scatterplot_ls10000)

# Save the plot as a high-resolution PNG
ggsave("Plots/form_scatterplot_ls10000 .png", plot = form_scatterplot_ls10000 , 
       width = 8, height = 6, dpi = 400)
```

#_th formal scatterplot>10000 allinone
```{r}
library(ggplot2)
library(tidyr)

# Filter for formal camps with N_INDVI < 10,000
formal_camps_gt10000 <- camps[camps$Sit_Stt == "Formal" & camps$N_INDVI > 10000, ]

formal_camps_gt10000_long <- formal_camps_gt10000  %>%
  pivot_longer(
    cols = c("gpw_UN", "ghsl", "wp_uc", "wp_c"),
    names_to = "Dataset",
    values_to = "Estimated_Population"
...
# Create a scatterplot with improved aesthetics
form_scatterplot_gt10000 <- ggplot(formal_camps_gt10000_long, 
                           aes(x = logas.numeric(N_INDVI), 
                               y = as.numeric(Estimated_Population), 
                               color = Dataset)) +
  geom_jitter(size = 0.8, alpha = 0.7) +  # Scatter points with jitter
  geom_smooth(method = "lm", se = FALSE) +  # Fit lines for each dataset
...
# Print the plot
print(form_scatterplot_gt10000)

# Save the plot as a high-resolution PNG
ggsave("Plots/form_scatterplot_gt10000 .png", plot = form_scatterplot_gt10000 , 
       width = 8, height = 6, dpi = 400)
```
...
# Filter for formal camps only
informal_camps <- camps[camps$Sit_Stt == "Informal", ]

# Combine the datasets into a long format for easier plotting
library(tidyr)

informal_camps_long <- informal_camps %>%
  pivot_longer(
    cols = c("gpw_UN", "ghsl", "wp_c", "wp_uc"),
    names_to = "Dataset",
    values_to = "Estimated_Population"
  )

# Create a scatterplot with different colored lines for each dataset
ggplot(informal_camps_long, aes(y = as.numeric(Estimated_Population), x = as.numeric(N_INDVI), color = Dataset)) +
  geom_jitter(size = 0.6, alpha = 0.8) +  # Scatter points
  geom_smooth(method = "lm", se = FALSE) +  # Add fit lines for each dataset
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +  # Diagonal line
  labs(
    title = "Comparison of Population Estimates for Informal Camps",
    y = "Estimated Population",
    x = "Survey Population",
    color = "Dataset"
  ) +
...
# Create a scatterplot with improved aesthetics
inform_scatterplot <- ggplot(informal_camps_long, 
                           aes(x = as.numeric(N_INDVI), 
                               y = as.numeric(Estimated_Population), 
                               color = Dataset)) +
  geom_jitter(size = 0.8, alpha = 0.7) +  # Scatter points with jitter
...
# Print the plot
print(inform_scatterplot)

# Save the plot as a high-resolution PNG
ggsave("Plots/inform_scatterplot.png", plot = inform_scatterplot, 
       width = 8, height = 6, dpi = 400)
```
...
# Calculate R² values for informal camps
r_squared_values_informal <- informal_camps_long %>%
  group_by(Dataset) %>%
  summarise(
    r_squared = cor(as.numeric(N_INDVI), as.numeric(Estimated_Population), use = "complete.obs")^2
  )

# Print R² values for informal camps
print(r_squared_values_informal)
...
#_the informal <10000 all in one
...
# Filter for formal camps with N_INDVI < 10,000
informal_camps_ls10000 <- camps[camps$Sit_Stt == "Informal" & camps$N_INDVI < 10000, ]

informal_camps_ls10000_long <- informal_camps_ls10000  %>%
  pivot_longer(
    cols = c("gpw_UN", "ghsl", "wp_uc", "wp_c"),
    names_to = "Dataset",
    values_to = "Estimated_Population"
...
# Create a scatterplot with improved aesthetics
inform_scatterplot_ls10000 <- ggplot(informal_camps_ls10000_long, 
                           aes(x = as.numeric(N_INDVI), 
                               y = as.numeric(Estimated_Population), 
                               color = Dataset)) +
  geom_jitter(size = 0.8, alpha = 0.7) +  # Scatter points with jitter
  ...
  scale_color_manual(values = dataset_colors) +  # Apply custom colors
  labs(
    title = "Comparison of Population Estimates for Informal Camps with population<10000",
    x = "Survey Population",
    y = "Estimated Population",
    color = "Dataset"
  ) +
...
# Print the plot
print(inform_scatterplot_ls10000)

# Save the plot as a high-resolution PNG
ggsave("Plots/inform_scatterplot_ls10000.png", plot = inform_scatterplot_ls10000 , 
       width = 8, height = 6, dpi = 400)
...
#_the informal>10000 all inone
```{r}
library(ggplot2)
library(tidyr)

# Filter for formal camps with N_INDVI < 10,000
informal_camps_gt10000 <- camps[camps$Sit_Stt == "Informal" & camps$N_INDVI > 10000, ]

informal_camps_gt10000_long <- informal_camps_gt10000  %>%
  pivot_longer(
    cols = c("gpw_UN", "ghsl", "wp_uc", "wp_c"),
    names_to = "Dataset",
    values_to = "Estimated_Population"
...
# Create a scatterplot with improved aesthetics
inform_scatterplot_gt10000 <- ggplot(informal_camps_gt10000_long, 
                           aes(x = as.numeric(N_INDVI), 
                               y = as.numeric(Estimated_Population), 
                               color = Dataset)) +
  geom_jitter(size = 0.8, alpha = 0.7) +  # Scatter points with jitter
...
# Print the plot
print(inform_scatterplot_gt10000)

# Save the plot as a high-resolution PNG
ggsave("Plots/inform_scatterplot_gt10000.png", plot = inform_scatterplot_gt10000 , 
       width = 8, height = 6, dpi = 400)
```
...
# Assuming 'camps' is your dataframe containing the relevant data
# and 'Sit_Stt' indicates the camp type (Formal or Informal)

# Gather the data into a long format for faceting
camps_long <- camps %>%
  select(Sit_Stt, N_INDVI, gpw_UN, ghsl, wp_uc, wp_c) %>%
  pivot_longer(cols = c(gpw_UN, ghsl, wp_uc, wp_c),
               names_to = "Dataset",
               values_to = "Estimated_Population")

# Create the faceted scatter plot
ggplot(camps_long, aes(x = N_INDVI, y = Estimated_Population, color = Sit_Stt)) +
  geom_point(alpha = 0.6) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
  facet_wrap(~ Dataset, scales = "free") +
  labs(title = "Survey vs. Estimated Population by Dataset and Camp Type",
       x = "Survey Population",
       y = "Estimated Population",
       color = "Camp Type") +
  theme_minimal() +
...
# Thesis used y=x corrected coordinate flipped
```{r}
# Revised scatter plot function
create_scatter_plot_residual <- function(data, y_var, title, min_y, max_y) {
  ggplot(data, aes_string(x = "N_INDVI", y = y_var, color = "Sit_Stt")) +
    geom_jitter(size = 0.6, alpha = 0.8) +  # Add points
...
# Find min and max for consistency across all datasets
min_value <- min(c(camps$wp_c, camps$wp_uc, camps$ghsl, camps$gpw_UN), na.rm = TRUE)
max_value <- max(c(camps$wp_c, camps$wp_uc, camps$ghsl, camps$gpw_UN), na.rm = TRUE)

# Create individual plots
scatter_gpw_resid <- create_scatter_plot_residual(camps, "gpw_UN", "GPW", min_value, max_value)
scatter_ghsl_resid <- create_scatter_plot_residual(camps, "ghsl", "GHS-POP", min_value, max_value)
scatter_wp_uncon_resid <- create_scatter_plot_residual(camps, "wp_uc", "WorldPop Unconstrained", min_value, max_value)
scatter_wp_con_resid <- create_scatter_plot_residual(camps, "wp_c", "WorldPop Constrained", min_value, max_value)

# Combine all plots into a 2x2 grid
combined_plot_residual <- grid.arrange(
  scatter_gpw_resid, scatter_ghsl_resid,
  scatter_wp_uncon_resid, scatter_wp_con_resid,
  nrow = 2, ncol = 2
)

# Display the combined plot
print(combined_plot_residual)
...
# Function to create heatmap with density of camps
create_heatmap <- function(data, x_var, title, min_x, max_x) {
  ggplot(data, aes_string(x = x_var, y = "N_INDVI")) +
    geom_bin2d(bins = 30) +  # Create the heatmap with density of camps
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +  # Add diagonal line
    ...
    theme(plot.title = element_text(hjust = 0.5, face = "bold"),
          legend.position = "bottom") +
    scale_x_continuous(limits = c(min_x, max_x)) +  # Set x-axis limits
    scale_y_continuous(limits = c(min_x, max_x))    # Set y-axis limits to match x-axis
}

# Heatmap for GPW
heatmap_gpw <- create_heatmap(camps, "gpw_UN", "GPW", min_value, max_value)

# Heatmap for GHS-POP
heatmap_ghsl <- create_heatmap(camps, "ghsl", "GHS-POP", min_value, max_value)

# Heatmap for WorldPop Unconstrained
heatmap_wp_uncon <- create_heatmap(camps, "wp_uc", "WorldPop Unconstrained", min_value, max_value)

# Heatmap for WorldPop Constrained
heatmap_wp_con <- create_heatmap(camps, "wp_c", "WorldPop Constrained", min_value, max_value)

# Arrange the heatmaps in the specified order using grid.arrange
combined_heatmap <- grid.arrange(
  heatmap_gpw, heatmap_ghsl,
  heatmap_wp_uncon, heatmap_wp_con,
  nrow = 2, ncol = 2
)
...
```{r}
ggsave("Plots/combined_plot_smooth_line_switch.png", plot = combined_plot_smooth_line_switch, width = 8, height = 6, dpi = 400)
```
```{r}
# Density plot for survey population
ggplot(camps, aes(x = N_INDVI, fill = Sit_Stt)) +
  geom_density(alpha = 0.5) +
  labs(x = "Survey Population", y = "Density")
...
# Function to create scatter plot with fit line and consistent x-axis scale (swapped x and y)
create_scatter_plot_with_fit <- function(data, x_var, title, min_x, max_x) {
  ggplot(data, aes_string(x = "N_INDVI", y = x_var, color = "Sit_Stt")) +
    geom_jitter(aes_string(x = "as.numeric(N_INDVI)", y = paste0("as.numeric(", x_var, ")")), 
                size = 0.6, alpha = 0.8) +
    geom_smooth(method = "lm", se = FALSE) +  # Add fit line
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +  # Add diagonal line
    labs(title = title,
         x = "Survey Population",  # Swapped axis labels
         y = "Estimated Population") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5, face = "bold"),
          legend.position = "none") +
    scale_y_continuous(limits = c(min_x, max_x))  # Set y-axis limits
}

# Scatter plot for GPW
scatter_gpw <- create_scatter_plot_with_fit(camps, "gpw_UN", "GPW (Gridded Population of the World)", min_value, max_value)

# Scatter plot for GHS-POP
scatter_ghsl <- create_scatter_plot_with_fit(camps, "ghsl", "GHS-POP (Global Human Settlement Population)", min_value, max_value)
...
# Arrange the plots in the specified order using grid.arrange
combined_plot_smooth_line_switch <- grid.arrange(
  scatter_gpw, scatter_ghsl,
  scatter_wp_uncon, scatter_wp_con,
  nrow = 2, ncol = 2
)
...
# Print the combined plot
print(combined_plot_smooth_line_switch)
...
# Combine all scatter plots into one
combined_plot_hh <- grid.arrange(scatter_nindvi_hh,scatter_gpw_UN_hh, scatter_wp_con_hh, scatter_wp_uncon_hh, scatter_ghsl_hh, nrow = 2, ncol = 3)
...
```{r}
# Find the minimum and maximum values of the estimated population variable across all datasets
min_value <- min(c(min( camps$wp_c, camps$wp_uc, camps$ghsl, camps$gpw_UN,camps$N_INDVI)))
max_value <- max(c(max(camps$wp_c, camps$wp_uc, camps$ghsl, camps$gpw_UN,camps$N_INDVI)))
...
# Arrange the plots using grid.arrange
combined_plot_smooth_line_hh <- grid.arrange(scatter_nind_nhh, scatter_gpw_UN,scatter_ghsl,scatter_wp_con, scatter_wp_uncon,  
                              nrow = 3, ncol = 2)
...
#one scatteplot with all the poputalion estimates and survey
...
# Assuming your data is in the sf object 'camps'
# Convert to data frame for ggplot2
camps_df <- as.data.frame(camps)

# Select the relevant columns and reshape the data
camps_long <- camps_df %>%
  select(N_INDVI, ghsl, wp_uc, wp_c, gpw_UN) %>%
  gather(key = "Population_Estimate", value = "Estimate_Value", -N_INDVI)

# Create the scatterplot
ggplot(camps_long, aes(x = N_INDVI, y = Estimate_Value, color = Population_Estimate)) +
  geom_point() +
  labs(x = "Survey Population (N_INDVI)", y = "Estimated Population",
       title = "Scatterplot of Survey Population vs. Estimated Populations") +
  theme_minimal()

```
# same above plot with nindvi less than and equal to 100
```{r}
# Load necessary libraries
library(sf)
library(ggplot2)
library(tidyr)
library(dplyr)  # for the filtering function
...
camps_long <- camps_df %>%
  select(N_INDVI, ghsl, wp_uc, wp_c, gpw_UN) %>%
  filter(N_INDVI <= 100) %>%  # Filter for N_INDVI <= 100
  gather(key = "Population_Estimate", value = "Estimate_Value", -N_INDVI)

# Create the scatterplot
ggplot(camps_long, aes(x = N_INDVI, y = Estimate_Value, color = Population_Estimate)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # 1:1 ratio line
  labs(x = "Survey Population (N_INDVI)", y = "Estimated Population",
       title = "Scatterplot of Survey Population vs. Estimated Populations (N_INDVI <= 100)") +
  theme_minimal()
...
```{r}
# Find the minimum and maximum values of the estimated population variable across all datasets
min_value <- min(c(min( subset_data$wp_c, subset_data$wp_uc, subset_data$ghsl, subset_data$gpw_UN)))
max_value <- max(c(max( subset_data$wp_c, subset_data$wp_uc, subset_data$ghsl, subset_data$gpw_UN)))
...
# Arrange the plots using grid.arrange
combined_plot_smooth_line_g1 <- grid.arrange( scatter_gpw_UN_g1,scatter_ghsl_g1,scatter_wp_con_g1, scatter_wp_uncon_g1,  
                              nrow = 3, ncol = 2)
...
```{r}
# Find the minimum and maximum values of the estimated population variable across all datasets
min_value <- min(c(min( subset_data_2$wp_c, subset_data_2$wp_uc, subset_data_2$ghsl, subset_data_2$gpw_UN)))
max_value <- max(c(max( subset_data_2$wp_c, subset_data_2$wp_uc, subset_data_2$ghsl, subset_data_2$gpw_UN)))
...
# Arrange the plots using grid.arrange
combined_plot_smooth_line_l1 <- grid.arrange( scatter_gpw_UN_l1,scatter_ghsl_l1,scatter_wp_con_l1, scatter_wp_uncon_l1,  
                              nrow = 2, ncol = 2)
...
# Find min and max for consistency across all datasets
min_value <- min(c(campls10000 $wp_c, campls10000 $wp_uc, campls10000 $ghsl, campls10000 $gpw_UN), na.rm = TRUE)
max_value <- max(c(campls10000 $wp_c, campls10000 $wp_uc, campls10000 $ghsl, campls10000 $gpw_UN), na.rm = TRUE)

# Create individual plots
scatter_gpw_resid <- create_scatter_plot_residual(campsgt10000, "gpw_UN", "GPW", min_value, max_value)
scatter_ghsl_resid <- create_scatter_plot_residual(campsgt10000, "ghsl", "GHS-POP", min_value, max_value)
scatter_wp_uncon_resid <- create_scatter_plot_residual(campsgt10000, "wp_uc", "WorldPop Unconstrained", min_value, max_value)
scatter_wp_con_resid <- create_scatter_plot_residual(campsgt10000, "wp_c", "WorldPop Constrained", min_value, max_value)

# Combine all plots into a 2x2 grid
combined_plot_residual_gt10000 <- grid.arrange(
  scatter_gpw_resid, scatter_ghsl_resid,
  scatter_wp_uncon_resid, scatter_wp_con_resid,
  nrow = 2, ncol = 2
)
...
# Display the combined plot
print(combined_plot_residual_gt10000)
```
...
# Find min and max for consistency across all datasets
min_value <- min(c(campsgt10000$wp_c, campsgt10000$wp_uc, campsgt10000$ghsl, campsgt10000$gpw_UN), na.rm = TRUE)
max_value <- max(c(campsgt10000$wp_c, campsgt10000$wp_uc, campsgt10000$ghsl, campsgt10000$gpw_UN), na.rm = TRUE)
...
```{r}
# Find the minimum and maximum values of the estimated population variable across all datasets
min_value <- min(c(min( campls10000 $wp_c, campls10000 $wp_uc, campls10000 $ghsl, campls10000 $gpw_UN)))
max_value <- max(c(max( campls10000$wp_c, campls10000$wp_uc, campls10000 $ghsl, campls10000$gpw_UN)))
...
# Arrange the plots using grid.arrange
combined_plot_smooth_line_pl1 <- grid.arrange( scatter_gpw_UN_pl1,scatter_ghsl_pl1,scatter_wp_con_pl1, scatter_wp_uncon_pl1,  
                              nrow = 2, ncol = 2)
...
# Find the minimum and maximum values of the estimated population variable across all datasets
min_value <- min(c(min( campsgt10000$wp_c, campsgt10000$wp_uc, campsgt10000$ghsl, campsgt10000$gpw_UN)))
max_value <- max(c(max( campsgt10000$wp_c, campsgt10000$wp_uc, campsgt10000$ghsl, campsgt10000$gpw_UN)))
...
# Arrange the plots using grid.arrange
combined_plot_smooth_line_pg1 <- grid.arrange( scatter_gpw_UN_pg1,scatter_ghsl_pg1,scatter_wp_con_pg1, scatter_wp_uncon_pg1,  
                              nrow = 2, ncol = 2)
...
# Check the data types again
str(camps[, c("N_INDVI", "ghsl", "wp_c", "wp_uc", "gpw_UN")])

# Check for missing values
summary(camps[, c("N_INDVI", "ghsl", "wp_c", "wp_uc", "gpw_UN")])
...
# [1] "GHSL: 0.669402024307656"
# [1] "wp_uc: 0.623507203215818"
# [1] "wp_c 0.548952555768835"
# [1] "gpw_UN: 0.151183078493222"
...
cor_matrix <- matrix(c(corr_ghsl, corr_wp_c, corr_wp_uc, corr_gpw_UN),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_INDVI"), c("ghsl", "wp_c", "wp_uc", "gpw_UN")))
...
numeric_cols <- c("N_INDVI","N_HH", "ghsl", "wp_c", "wp_uc", "gpw_UN")
...
N_INDVI and N_HH (0.940): There is a very strong positive correlation between the survey population and the number of households. This suggests that as the survey population increases, the number of households also increases almost proportionately.
N_HH and ghsl (0.694): There is a strong positive correlation between the number of households and the global human settlement layer. This implies that areas with more households are strongly related to the density or presence of human settlements.
ghsl and wp_uc (0.890): A very strong positive relationship between the global human settlement layer and urban classification, indicating that regions classified as human settlements are predominantly urban areas.
Moderate Positive Correlations:
N_INDVI and ghsl (0.669): A moderate positive correlation between the survey population and the global human settlement layer. This suggests that areas with higher survey populations tend to have more significant human settlement presence.
N_INDVI and wp_c (0.549): A moderate positive correlation between the survey population and WorldPop population count, indicating a reasonable alignment between survey population numbers and WorldPop estimates.
ghsl and wp_c (0.604): A moderate positive correlation between human settlements and WorldPop population count, suggesting a decent alignment between human settlement data and population estimates.
wp_c and wp_uc (0.661): A moderate positive correlation between WorldPop population count and urban classification, indicating that higher population counts are often found in urban areas.
Lower Positive Correlations:
wp_uc and gpw_UN (0.191): A low positive correlation between WorldPop urban classification and GPW UN population, suggesting some relationship but not very strong.
N_INDVI and gpw_UN (0.151): A low positive correlation between the survey population and GPW UN population, indicating some alignment but not strong.
N_HH and gpw_UN (0.097): A very low positive correlation between the number of households and GPW UN population, suggesting minimal relationship.
ghsl and gpw_UN (0.131): A low positive correlation between human settlements and GPW UN population, indicating some level of alignment but weak.
Noteworthy Observations:
wp_c and gpw_UN (0.744): A strong positive correlation between WorldPop population count and GPW UN population, indicating that these two datasets are in strong agreement regarding population estimates.
Summary:
The survey population (N_INDVI) has a very strong correlation with the number of households (N_HH), indicating that household numbers are a reliable indicator of the survey population. There is also a notable correlation between the survey population and the global human settlement layer (ghsl), as well as a moderate correlation with WorldPop population counts (wp_c). Lower correlations with GPW UN population data suggest some differences in how populations are accounted for or estimated in these datasets compared to the survey data.
...
rownames(corr_matrix) <- colnames(corr_matrix) <- c("N_INDVI","N_HH", "ghsl", "wp_con", "wp_uncon", "gpw_UN")
...
#HEAT MAP correlation with coefficient of detemination
```{r}
library(dplyr)
library(reshape2)
library(ggplot2)
...
# Convert the R^2 matrix to a dataframe
r_squared_matrix_long <- melt(r_squared_matrix)

# Assign row and column names for clarity
rownames(r_squared_matrix) <- colnames(r_squared_matrix) <- c("N_INDVI", "N_HH", "ghsl", "wp_con", "wp_uncon", "gpw_UN")

# Plot the heatmap with R^2 values
heat_map_r_squared <- ggplot(data = r_squared_matrix_long, aes(x = Var1, y = Var2, fill = value, label = round(value, 2))) +
  geom_tile(color = "white") +
  geom_text(color = "black", size = 2) +
  scale_fill_gradient(low = "lightblue", high = "lightcoral") +
  labs(title = "Coefficient of Determination (R^2) Heatmap",
       x = "Variables",
       y = "Variables") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Print the heatmap
print(heat_map_r_squared)

```
```{r}
# Save the population distrib plot to a file
ggsave("Plots/heat_map_r_squared.png", plot =heat_map_r_squared, width = 8, height = 6, dpi = 400)
```
...
# [1] "GHSL: 0.00690077041185157"
# [1] "wp_uc: 0.220784729400368"
# [1] "wp_c 0.104199783346568"
# [1] "gpw_UN: 0.0436065237844927"
```
...
```{r}
# Create a matrix of correlation coefficients
cor_matrix_areal <- matrix(c(corr_ghsl_areal, corr_wp_c_areal, corr_wp_uc_areal ,corr_gpw_UN_areal),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_INDVI"), c("ghsl", "wp_c", "wp_uc", "gpw_UN")))
...
# [1] "GHSL: 0.641742641716055"
# [1] "wp_uc: 0.5909897941457"
# [1] "wp_c 0.502923085012834"
# [1] "gpw_UN: 0.107084792910909"
```
...
```{r}
# Create a matrix of correlation coefficients
cor_matrix_areag <- matrix(c(corr_ghsl_areag, corr_wp_c_areag, corr_wp_uc_areag, corr_gpw_UN_areag),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_INDVI"), c("ghsl", "wp_c", "wp_uc", "gpw_UN")))
...
# [1] "GHSL: 0.509565129232103"
# [1] "wp_uc: 0.381366770819244"
# [1] "wp_c 0.212715927904344"
# [1] "gpw_UN: -0.134095801917998"
```

```{r}
# Create a matrix of correlation coefficients
cor_matrix_pg <- matrix(c(corr_ghsl_pg, corr_wp_c_pg, corr_wp_uc_pg,corr_gpw_UN_pg),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_INDVI"), c("ghsl", "wp_c", "wp_uc", "gpw_UN")))
...
formal_camps <- campls10000[campls10000$Sit_stt == "Formal", ]
informal_camps <- campls10000[campls10000$Sit_stt == "Informal", ]
# Calculate correlation coefficients
corr_ghsl_pl <- cor(formal_camps$N_INDVI,formal_camps$ghsl,method=("pearson"))
corr_wp_c_pl <-cor(formal_camps$N_INDVI,formal_camps$wp_c)
corr_wp_uc_pl <-cor(formal_camps$N_INDVI,formal_camps$wp_uc)
# corr_gpw_pl <-cor(campls10000$N_INDVI,campls10000$gpw)
corr_gpw_UN_pl <-cor(formal_camps$N_INDVI,formal_camps$gpw_UN)
# View correlation coefficients
...
# [1] "GHSL: 0.407094645924059"
# [1] "wp_uc: 0.510279634549507"
# [1] "wp_c 0.310321687917034"
# [1] "gpw_UN: 0.0711666429556592"
...
campls10000$wp_c <- as.numeric(campls10000$wp_c)
campls10000$wp_uc <- as.numeric(campls10000$wp_uc)
campls10000$gpw_UN <- as.numeric(campls10000$gpw_UN)

# Subset data for formal and informal camps
formal_camps <- campls10000[campls10000$Sit_Stt == "Formal", ]
informal_camps <- campls10000[campls10000$Sit_Stt == "Informal", ]
...
calculate_correlation <- function(data) {
  list(
    corr_ghsl = cor(data$N_INDVI, data$ghsl, method = "pearson"),
    corr_wp_c = cor(data$N_INDVI, data$wp_c, method = "pearson"),
    corr_wp_uc = cor(data$N_INDVI, data$wp_uc, method = "pearson"),
    corr_gpw_UN = cor(data$N_INDVI, data$gpw_UN, method = "pearson")
  )
}

# Correlations for formal camps (population <10,000)
formal_camps_subset <- formal_camps[formal_camps$N_INDVI < 10000, ]
formal_corr <- calculate_correlation(formal_camps_subset)

# Correlations for informal camps (population <10,000)
informal_camps_subset <- informal_camps[informal_camps$N_INDVI < 10000, ]
informal_corr <- calculate_correlation(informal_camps_subset)

# Print correlation results
print("Formal Camps Correlations:")
print(formal_corr)

print("Informal Camps Correlations:")
print(informal_corr)
...
```{r}
# Create a matrix of correlation coefficients
cor_matrix_pl <- matrix(c(corr_ghsl_pl, corr_wp_c_pl, corr_wp_uc_pl, corr_gpw_UN_pl),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_INDVI"), c("ghsl", "wp_c", "wp_uc", "gpw_UN")))
...
```{r}
# Create a matrix of correlation coefficients
cor_matrix_f <- matrix(c(corr_ghsl_f, corr_wp_c_f, corr_wp_uc_f, corr_gpw_UN_f),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_INDVI"), c("ghsl", "wp_c", "wp_uc", "gpw_UN")))
...
```{r}
# Create a matrix of correlation coefficients
cor_matrix_inf <- matrix(c(corr_ghsl_inf, corr_wp_c_inf, corr_wp_uc_inf, corr_gpw_UN_inf),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_INDVI"), c("ghsl", "wp_c", "wp_uc", "gpw_UN")))
...
# Check the data types again
str(camps[, c("N_HH", "ghsl", "wp_c", "wp_uc", "gpw_UN")])
...
# [1] "GHSL: 0.693588796841264"
# [1] "wp_uc: 0.616542204420976"
# [1] "wp_c 0.520772828563885"
# [1] "gpw_UN: 0.0968625748524237"
```

```{r}
# Create a matrix of correlation coefficients
cor_matrix_hh <- matrix(c(corr_ghsl_HH, corr_wp_c_HH, corr_wp_uc_HH,corr_gpw_UN_HH),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_HH"), c("ghsl", "wp_c", "wp_uc", "gpw_UN")))
...
subset_data_2$wp_c <- as.numeric(subset_data_2$wp_c)
subset_data_2$wp_uc<- as.numeric(subset_data_2$wp_uc)
subset_data_2$gpw_UN <- as.numeric(subset_data_2$gpw_UN)
...
# [1] "GHSL: 0.0514408723002811"
# [1] "wp_uc: 0.225983847913498"
# [1] "wp_c 0.187571646495442"
# [1] "gpw_UN: 0.102660033468945"
```
...
```{r}
# Create a matrix of correlation coefficients
cor_matrix_areal_hh <- matrix(c(corr_ghsl_areal_hh, corr_wp_c_areal_hh, corr_wp_uc_areal_hh, corr_gpw_UN_areal_hh),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_HH"), c("ghsl", "wp_c", "wp_uc","gpw_UN")))
...
subset_data$wp_c <- as.numeric(subset_data$wp_c)
subset_data$wp_uc<- as.numeric(subset_data$wp_uc)
subset_data$gpw_UN <- as.numeric(subset_data$gpw_UN)
...
# 
# [1] "GHSL: 0.669011698727262"
# [1] "wp_uc: 0.584882295136978"
# [1] "wp_c 0.474898662853394"
# [1] "gpw: 0.0523313972591817"
# [1] "gpw_UN: 0.0521537049762277"
```
...
```{r}
# Create a matrix of correlation coefficients
cor_matrix_areag_hh <- matrix(c(corr_ghsl_areag_hh, corr_wp_c_areag_hh, corr_wp_uc_areag_hh, corr_gpw_UN_areag_hh),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_HH"), c("ghsl", "wp_c", "wp_uc","gpw_UN")))
...
campsgt10000$wp_c <- as.numeric(campsgt10000$wp_c)
campsgt10000$wp_uc<- as.numeric(campsgt10000$wp_uc)
campsgt10000$gpw_UN <- as.numeric(campsgt10000$gpw_UN)
...
```{r}
# Create a matrix of correlation coefficients
cor_matrix_pg_hh <- matrix(c(corr_ghsl_pg_hh, corr_wp_c_pg_hh, corr_wp_uc_pg_hh, corr_gpw_UN_pg_hh),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_HH"), c("ghsl", "wp_c", "wp_uc","gpw_UN")))
...
# 1] "GHSL: 0.303260527306786"
# [1] "wp_uc: 0.332038034502622"
# [1] "wp_c 0.211690642209158"
# [1] "gpw: 0.0316770805697565"
# [1] "gpw_UN: 0.0316770805697565"

```

```{r}
# Create a matrix of correlation coefficients
cor_matrix_pl_hh <- matrix(c(corr_ghsl_pl_hh, corr_wp_c_pl_hh, corr_wp_uc_pl_hh, corr_gpw_UN_pl_hh),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_HH"), c("ghsl", "wp_c", "wp_uc", "gpw_UN")))
...
# [1] "GHSL: 0.710763796377918"
# [1] "wp_uc: 0.611933291514018"
# [1] "wp_c 0.550940469244188"
# [1] "gpw_UN: 0.113474859187085"
```

```{r}
# Create a matrix of correlation coefficients
cor_matrix_hh_f <- matrix(c(corr_ghsl_HH_f, corr_wp_c_HH_f, corr_wp_uc_HH_f, corr_gpw_UN_HH_f),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_HH"), c("ghsl", "wp_c", "wp_uc","gpw_UN")))
...
# 
# [1] "GHSL: 0.655724406371716"
# [1] "wp_uc: 0.707478052641468"
# [1] "wp_c 0.45940791381195"
# [1] "gpw_UN: 0.1301776717643"
```

```{r}
# Create a matrix of correlation coefficients
cor_matrix_hh_if <- matrix(c(corr_ghsl_HH_if, corr_wp_c_HH_if, corr_wp_uc_HH_if, corr_gpw_UN_HH_if),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_HH"), c("ghsl", "wp_c", "wp_uc","gpw_UN")))
...
#POPULATION as box plot
```{r}
# Combine all population difference columns into one data frame
pop_data <- data.frame(
  Dataset = rep(c("WP_CON", "WP_UNCON",  "GHSL", "GPW_UN","N_INDVI"), each = nrow(camps)),
  Population = c(as.numeric(camps$wp_c), as.numeric(camps$wp_uc), as.numeric(camps$ghsl),as.numeric(camps$gpwU),as.numeric(camps$N_INDVI))
)

# Calculate mean values
mean_values <- aggregate(Population ~ Dataset, data = pop_data, mean)

# Create a box plot
boxplot_pop <- ggplot(pop_data, aes(x = Dataset, y = Population)) +
  geom_boxplot(fill = "skyblue", color = "black") +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 1, color = "red") + # Adding mean points
  geom_text(data = mean_values, aes(label = round(Population, digits = 2)), vjust = -0.5,size=2) + # Adding text labels
  labs(title = "Population estimates for different global data products",
       x = "Dataset", y = "Population") +
  theme_minimal() +
 theme(axis.text.x = element_text(angle = 45, hjust = 1), 
...
#thesis data needed here starts
...
difference_data <- data.frame(
  Dataset = rep(c("GPW-V4","GHS-POP","WorldPop_unconstrained","WorldPop_Constrained"), each = nrow(camps)),
  Population_Difference = c(as.numeric(camps$gpwU_df), as.numeric(camps$ghsl_df), as.numeric(camps$wp_uc_df),as.numeric(camps$wp_c_df))
)
# to keep the order of gpw, ghs, wpun, wpcon
# Ensure the correct order of the factors for the Dataset variable
difference_data$Dataset <- factor(difference_data$Dataset, levels = c("GPW-V4","GHS-POP","WorldPop_unconstrained","WorldPop_Constrained"))
...
boxplot_pop_diff <- ggplot(difference_data, aes(x = Dataset, y = Population_Difference)) +
 ...
 theme(axis.text.x = element_text(angle = 0, hjust = 0.5), 
        plot.title = element_text(size = 10))+  # Focus on a specific range without altering data
  scale_x_discrete(labels = function(x) str_wrap(x, width = 7))  # Wrap x-axis labels to width of 7 Rotate x-axis labels
...
```{r}
# Save the population distrib plot to a file
ggsave("Plots/boxplot_pop.png", plot =boxplot_pop, width = 8, height = 6, dpi = 400)


# Save the boxplot diff plot to a file
...
difference_data_formal <- data.frame(
  Dataset = rep(c("WP_CON", "WP_UNCON",  "GHSL", "GPW_UN"), each = nrow(formal_camps)),
  Population_Difference = c(as.numeric(formal_camps$wp_c_df), as.numeric(formal_camps$wp_uc_df),as.numeric(formal_camps$ghsl_df),as.numeric(formal_camps$gpwU_df))
)
...
difference_data_pl <- data.frame(
  Dataset = rep(c("WP_CON", "WP_UNCON",  "GHSL", "GPW_UN"), each = nrow(campls10000)),
  Population_Difference = c(as.numeric(campls10000$wp_c_df), as.numeric(campls10000$wp_uc_df), as.numeric(campls10000$ghsl_df),as.numeric(campls10000$gpwU_df))
)
...
summary_data <- data.frame(
  Dataset = c("GPW-V4","GHS-POP","WorldPop_unconstrained","WorldPop_Constrained"),
  Mean_Absolute_Difference = c(mean(absolute_difference), mean(absolute_difference_gpw_UN), 
...
#swapping columns to make sure wp_c and wp_uc have correct values

```{r}
# Save the column names of camps_wp_uc and camps_wp_c
temp_colnames_wp_uc <- colnames(camps$wp_uc)
temp_colnames_wp_c <- colnames(camps$wp_c)

# Swap the column names
colnames(camps$wp_uc) <- temp_colnames_wp_c
colnames(camps$wp_c) <- temp_colnames_wp_uc
...
#trying to make the area beteween 0 and 1 significant

```{r}
# Create the box plot with adjusted y-axis limits and outlier color, and multiply y-axis by -1
boxplot_ratio <- ggplot(difference_data, aes(x = Dataset, y = -ratio)) +
  geom_boxplot(fill = "lightblue", color = "black", outlier.color = "lightgray") +
  stat_summary(fun = mean, geom = "point", shape = 3, size = 1, color = "blue") +
  geom_text(data = mean_ratio_values, aes(label = round(-ratio, digits = 2)), 
            vjust = -0.5, size = 2, color = 'blue') +  # Note: -ratio to reflect the y-axis flip
  labs(title = "Ratio of Difference to Survey Population for Different EO Population Data Products",
       x = "Dataset", y = "Ratio of Difference to Survey Population") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(size = 10)) +
  scale_y_continuous(limits = c(-1.5, .5))  # Adjust the y-axis limits to focus on the box region

# Print the adjusted box plot
print(boxplot_ratio)
...
# Aggregate mean ratio values
mean_ratio_values <- aggregate(ratio ~ Dataset, data = difference_data, median)
...
boxplot_ratio_interactive <- plot_ly(difference_data, x = ~Dataset, y = ~ratio, type = "box") %>%
  add_markers(text = ~paste("Dataset:", Dataset, "<br>Name:", camps$Name, "<br>Sit_Stt:", camps$Sit_Stt, "<br>cl_Shp_a:", camps$cl_Shp_a,
   "<br>campname:", camps$Site_nm,                         "<br>Ratio:", ratio)) %>%
...
```
# ___thesis used
# Create the box plot while zooming in on the range of interest and ensuring all data is considered
```{r}
library(stringr)
# Create the box plot while zooming in on the range of interest and ensuring all data is considered
boxplot_ratio <- ggplot(difference_data, aes(x = Dataset, y = -ratio)) +
  ...
  stat_summary(fun = median, geom = "point", shape = 23, size = 1, color = "red", fill = "yellow") +
  # Add the median value as text
  stat_summary(fun = median, geom = "text", aes(label = round(..y.., digits = 2)), 
               vjust = -0.5, size = 3, color = 'black') +
  labs(
       x = "Dataset", y = "Ratio of Difference of estimate to Survey Population") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 0.5)
  ) +
  coord_cartesian(ylim = c(-1.5, 0.5)) +  # Focus on a specific range without altering data
  scale_x_discrete(labels = function(x) str_wrap(x, width = 7))  # Wrap x-axis labels to width of 7
# Print the adjusted box plot
print(boxplot_ratio)
```
...
```{r}
# Save the plot to a file
ggsave("Plots/boxplot_ratio.png", plot =boxplot_ratio, width = 8, height = 5, dpi = 400)
```
...
```
#thesis used
#boxplotratio shows formal informal

```{r}
# Combine all population difference columns into one data frame
difference_data_stt <- data.frame(
  Dataset = rep(c("GPW-v4", "GHS-POP", "WorldPop_unconstrained", "WorldPop_constrained"), each = nrow(camps)),
  Population_Difference_stt = c(as.numeric(camps$gpwU_df), as.numeric(camps$ghsl_df), as.numeric(camps$wp_uc_df), as.numeric(camps$wp_c_df)),
  Sit_Stt = camps$Sit_Stt  # Adding Sit_Stt column
)

# Ensure the correct order of the factors for the Dataset variable
difference_data_stt$Dataset <- factor(
  difference_data_stt$Dataset, 
  levels = c("GPW-v4", "GHS-POP", "WorldPop_unconstrained", "WorldPop_constrained")
)

# Calculate the ratio of the difference to the surveyed population
difference_data_stt$ratio <- difference_data_stt$Population_Difference_stt / camps$N_INDVI

# Create the boxplot
boxplot_ratio_stat <- ggplot(difference_data_stt, aes(x = Dataset, y = -ratio, fill = Sit_Stt)) +
  geom_boxplot(color = "black", outlier.color = "lightgray") +  # Box plot with outlier color
  # Add a point for the median
  ...
  stat_summary(
    ...
    color = "red", 
    fill = "yellow", 
    position = position_dodge(width = 0.75)  # Dodge to separate Formal and Informal
  )+
  # Add the median value as text for Formal and Informal
  stat_summary(
    fun = median, 
    geom = "text", 
    aes(label = round(..y.., digits = 2), group = Sit_Stt), 
  ...
  labs(
    #title = "Ratio of Difference to Survey Population by EO Population Data Products and Site Status",
    x = "Dataset", 
    y = "Ratio of Difference to Survey Population", 
    fill = "Site Status"
  ) +
  theme_minimal() +
  theme(
...
# Print the adjusted box plot
print(boxplot_ratio_stat)

```
#percentage of underestimation, overestimation
```{r}
# Overall underestimation and overestimation
underestimation_count <- sum(difference_data_stt$ratio < 0, na.rm = TRUE)
overestimation_count <- sum(difference_data_stt$ratio > 0, na.rm = TRUE)
total_count <- nrow(difference_data_stt)

# Overall percentages
underestimation_percentage <- (underestimation_count / total_count) * 100
overestimation_percentage <- (overestimation_count / total_count) * 100

# Print overall results
cat("Overall Results:\n")
cat("Percentage of data that underestimates the surveyed population:", underestimation_percentage, "%\n")
cat("Percentage of data that overestimates the surveyed population:", overestimation_percentage, "%\n\n")

# Per-dataset underestimation and overestimation
library(dplyr)

dataset_stats <- difference_data_stt %>%
  group_by(Dataset) %>%
  summarise(
    Underestimation_Count = sum(ratio < 0, na.rm = TRUE),
    Overestimation_Count = sum(ratio > 0, na.rm = TRUE),
    Total_Count = n(),
    Underestimation_Percentage = (Underestimation_Count / Total_Count) * 100,
    Overestimation_Percentage = (Overestimation_Count / Total_Count) * 100,
    .groups = "drop"
...
```
#thesis percentatge of under or overestimation of formal and informal

```{r}
# Create a function to calculate underestimation and overestimation percentages
calculate_estimation_stats <- function(data) {
  underestimation_count <- sum(data$ratio < 0, na.rm = TRUE)
  overestimation_count <- sum(data$ratio > 0, na.rm = TRUE)
...
# Split data by Dataset and Site Status
library(dplyr)

estimation_stats <- difference_data_stt %>%
  group_by(Dataset, Sit_Stt) %>%
  summarise(
    Underestimation_Percentage = sum(ratio < 0, na.rm = TRUE) / n() * 100,
    Overestimation_Percentage = sum(ratio > 0, na.rm = TRUE) / n() * 100,
    Total_Count = n(),
    .groups = "drop"
  )

# Print the results
print(estimation_stats)

write.csv(estimation_stats, "Plots/estimation_stats_formala_inf.csv", row.names = FALSE)
...
# Create the base ggplot (same as the static version)
boxplot_ratio_stat <- ggplot(difference_data_stt, aes(x = Dataset, y = -ratio, fill = Sit_Stt)) +
  geom_boxplot(color = "black", outlier.color = "lightgray") +  # Box plot with outlier color
  stat_summary(
    fun = median, 
...
interactive_plot <- ggplotly(boxplot_ratio_stat)
...
save ratio error formnal-informal plot

```{r}
# Save the plot to a file
ggsave("Plots/boxplot_ratio_stat.png", plot =boxplot_ratio_stat, width = 8, height = 6, dpi = 400)
```
...
difference_data_stt <- data.frame(
  Dataset = rep(c("GPW-v4", "GHS-POP", "WorldPop_unconstrained", "WorldPop_constrained"), each = nrow(camps)),
  Population_Difference_stt = c(as.numeric(camps$gpwU_df), as.numeric(camps$ghsl_df), as.numeric(camps$wp_uc_df), as.numeric(camps$wp_c_df)),
  Shlt_ty = camps$Shlt_ty  # Adding Sit_Stt column
)
...
# Create the boxplot
boxplot_ratio_stat <- ggplot(difference_data_stt, aes(x = Dataset, y = -ratio, fill =  Shlt_ty)) +
  geom_boxplot(color = "black", outlier.color = "lightgray") +  # Box plot with outlier color
  # Add a point for the median
  ...
  stat_summary(
    fun = median, 
    geom = "text", 
    aes(label = round(..y.., digits = 2), group = Shlt_ty), 
    position = position_dodge(width = 0.5), 
    vjust = -0.5, 
    size = 2, 
    color = 'black '
  ...
  coord_cartesian(ylim = c(-1.5, 1.5)) +  # Focus on a specific range without altering data
  scale_x_discrete(labels = function(x) str_wrap(x, width = 7))  # Wrap x-axis labels to width of 7
...
```{r}
# Refined Boxplot Code
library(ggplot2)
library(stringr)

# Create the boxplot with refinements
refined_boxplot <- ggplot(difference_data_stt, aes(x = Dataset, y = -ratio, fill = Shlt_ty)) +
  geom_boxplot(color = "black", outlier.size = 1, outlier.alpha = 0.6) +  # Adjust outlier size and transparency
  stat_summary(
    fun = median, 
    geom = "point", 
    aes(group = Shlt_ty), 
  ...
  labs(
    title = "Comparison of Population Estimate Errors Across Shelter Types",
    subtitle = "Analyzing ratio of differences between estimated and survey populations",
    x = "Dataset", 
    y = "Ratio of Population Estimate Difference to Survey Population", 
    fill = "Shelter Type"
  ) +
  theme_minimal() +
  theme(
  ...
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10))  # Adjust label width

# Print the refined boxplot
print(refined_boxplot)

```
#thesis shelter type boxplot
```{r}
# Load necessary libraries
library(ggplot2)
library(stringr)
...
# Filter out 'Collective Settlement/Centre' from the data
filtered_data <- difference_data_stt %>%
  filter(Shlt_ty != "Collective Settlement/Centre")

# Create the boxplot with refinements
refined_boxplot <- ggplot(filtered_data, aes(x = Dataset, y = -ratio, fill = Shlt_ty)) +
  geom_boxplot(color = "black", outlier.size = 1, outlier.alpha = 0.6) +  # Adjust outlier size and transparency
  stat_summary(
    fun = median, 
    geom = "point", 
...
# Create the boxplot arranged by shelter type
refined_boxplot_by_shelter <- ggplot(filtered_data, aes(x = Shlt_ty, y = -ratio, fill = Dataset)) +
  geom_boxplot(color = "black", outlier.size = 1, outlier.alpha = 0.6, position = position_dodge(width = 0.75)) +
  stat_summary(
    fun = median, 
    geom = "point", 
    aes(group = Dataset), 
  ...
  stat_summary(
    fun = median, 
    geom = "text", 
    aes(label = round(..y.., digits = 2), group = Dataset), 
    position = position_dodge(width = 0.75), 
    vjust = -0.7, 
    size = 2.5, 
    color = "black"
  ...
  scale_fill_brewer(palette = "Set2") +  # Use a colorblind-friendly palette
  labs(
    title = "Comparison of Population Estimate Errors by Shelter Type",
    subtitle = "Analyzing ratio of differences between estimated and surveyed populations",
    x = "Shelter Type", 
    y = "Ratio of Population Estimate Difference to Survey Population", 
    fill = "Dataset"
  ) +
  ...
  coord_cartesian(ylim = c(-1.3, 1)) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 5))  # Adjust label width

# Print the refined boxplot
print(refined_boxplot_by_shelter)

```

```{r}
# Save the plot to a file
ggsave("Plots/refined_boxplot_by_shelter.png", plot =refined_boxplot_by_shelter, width = 10, height = 6, dpi = 500)
```

# percentage underestimation and overesteimation or correct for shelter type
```{r}
# Calculate underestimation, overestimation, and correct estimation by Shelter Type
shelter_type_stats <- filtered_data %>%
  group_by(Shlt_ty) %>%
  summarise(
    Underestimation_Count = sum(ratio < 0, na.rm = TRUE),
    Overestimation_Count = sum(ratio > 0, na.rm = TRUE),
    Correct_Estimation_Count = sum(ratio == 0, na.rm = TRUE),
    Total_Count = n(),
    Underestimation_Percentage = (Underestimation_Count / Total_Count) * 100,
    Overestimation_Percentage = (Overestimation_Count / Total_Count) * 100,
    Correct_Estimation_Percentage = (Correct_Estimation_Count / Total_Count) * 100,
    .groups = "drop"
  )

# Print the results
print(shelter_type_stats)

```
# pecentage under/over estim shelter type
```{r}
# Define a function to calculate estimation stats for a dataset
...
shelter_stats_ghs <- calculate_stats(filtered_data_ghs, "GHS-POP")
shelter_stats_wpc <- calculate_stats(filtered_data_wpc, "WorldPop Constrained")
shelter_stats_wpuc <- calculate_stats(filtered_data_wpuc, "WorldPop Unconstrained")
shelter_stats_gpw <- calculate_stats(filtered_data_gpw, "GPW")

# Combine results from all datasets
all_shelter_stats <- bind_rows(shelter_stats_ghs, shelter_stats_wpc, shelter_stats_wpuc, shelter_stats_gpw)

# View results
print(all_shelter_stats)
...
# Calculate statistics for each shelter type
shelter_type_stats <- filtered_data %>%
  group_by(Shlt_ty) %>%
  summarise(
    Underestimation_Count = sum(ratio < 0, na.rm = TRUE),
...
# Filter only numeric columns (exclude any spatial or non-numeric data)
radar_data <- shelter_type_stats %>%
  select(Shlt_ty, Underestimation_Percentage, Overestimation_Percentage, Correct_Estimation_Percentage)

# Ensure all percentage columns are numeric (explicit conversion)
radar_data <- radar_data %>%
  mutate(
    Underestimation_Percentage = as.numeric(Underestimation_Percentage),
    Overestimation_Percentage = as.numeric(Overestimation_Percentage),
    Correct_Estimation_Percentage = as.numeric(Correct_Estimation_Percentage)
  )

# Check for any NA values in the numeric columns
if(any(is.na(radar_data[,2:4]))) {
  print("There are NA values in the radar chart data.")
  print(radar_data)  # Inspect the data to see which rows contain NA
  stop("NA values detected.")
}

# Reshape data to long format for radar chart
radar_data_long <- radar_data %>%
  gather(key = "Estimation_Type", value = "Percentage", -Shlt_ty)

# Prepare data for the radar chart using pivot_wider instead of spread()
radar_data_ready <- radar_data_long %>%
  pivot_wider(names_from = Estimation_Type, values_from = Percentage)

# Add a row with maximum values to scale the radar chart
max_values <- data.frame(
  Shlt_ty = "Max",
  Underestimation_Percentage = 100,
  Overestimation_Percentage = 100,
  Correct_Estimation_Percentage = 100
)

# Append the max values row
radar_data_ready <- bind_rows(radar_data_ready, max_values)

# Ensure all data is numeric before creating the radar chart
radar_data_ready[,2:4] <- lapply(radar_data_ready[,2:4], as.numeric)

# Check if there are any NA values left before plotting
if(any(is.na(radar_data_ready[,2:4]))) {
  print("There are still NA values in the radar chart data.")
  print(radar_data_ready)  # Inspect the final dataset
  stop("NA values detected.")
}

# Create radar chart using fmsb package
radarchart(radar_data_ready, axistype = 1,
           pcol = rgb(0.2, 0.5, 0.5, 0.9), pfcol = rgb(0.2, 0.5, 0.5, 0.5), plwd = 4, plty = 1,
           axislabcol = "gray", cglcol = "gray", cglty = 1, cglwd = 0.8,
           vlcex = 0.8)

# Adding title
title(main = "Estimation Accuracy by Shelter Type", cex.main = 1.5)

```

```{r}
# Calculate underestimation, overestimation, and correct estimation for each dataset and shelter type
dataset_shelter_stats <- filtered_data %>%
  group_by(Dataset, Shlt_ty) %>%
  summarise(
    Underestimation_Count = sum(ratio < 0, na.rm = TRUE),
    Overestimation_Count = sum(ratio > 0, na.rm = TRUE),
    Correct_Estimation_Count = sum(ratio == 0, na.rm = TRUE),
...
# Print the results
print(dataset_shelter_stats)
write.csv(dataset_shelter_stats, "Plots/shelter_type_stats.csv", row.names = FALSE)


```

```{r}
# Filter data for "Emergency Shelter" and "Self-made/Makeshift Shelter"
filtered_data <- subset(difference_data_stt, Shlt_ty %in% c("Emergency Shelter", "Self-made/Makeshift Shelter"))
...
refined_boxplot <- ggplot(filtered_data, aes(x = Dataset, y = -ratio, fill = Shlt_ty)) +
  ...
  scale_fill_brewer(palette = "Set2") +  # Use a colorblind-friendly palette
  labs(
    title = "Comparison of Population Estimate Errors for Specific Shelter Types",
    subtitle = "Analyzing ratio of differences between estimated and survey populations",
    x = "Dataset", 
    y = "Ratio of Population Estimate Difference to Survey Population", 
    fill = "Shelter Type"
...
# Custom color palette for the selected shelter types
custom_colors <- c("Emergency Shelter" = "#7A4B9C",  # Blue for Emergency Shelter
                   "Self-made/Makeshift Shelter" = "#FFCC00")  # Green for Makeshift Shelter
...
```{r}
# Save the plot to a file
ggsave("Plots/refined_boxplot_with_c.png", plot =refined_boxplot, width = 10, height = 6, dpi = 500)
```
...
# Assuming your data is in the sf object 'camps'
# Convert to data frame for calculation
camps_df <- as.data.frame(camps)

# Calculate means using the entire dataset
mean_survey <- mean(camps_df$N_INDVI, na.rm = TRUE)
mean_wp_uc <- mean(camps_df$wp_uc, na.rm = TRUE)
mean_wp_c <- mean(camps_df$wp_c, na.rm = TRUE)
mean_gpw <- mean(camps_df$gpw_UN, na.rm = TRUE)
mean_ghsl <- mean(camps_df$ghsl, na.rm = TRUE)

# Calculate biases
bias_wp_uc <- mean_wp_uc - mean_survey
bias_wp_c <- mean_wp_c - mean_survey
bias_gpw <- mean_gpw - mean_survey
bias_ghsl <- mean_ghsl - mean_survey

# Create a data frame to display the biases
bias_df <- data.frame(
  Method = c("wp_uc", "wp_c", "gpw", "ghsl"),
  Bias = c(bias_wp_uc, bias_wp_c, bias_gpw, bias_ghsl)
)
...
# Create a data frame with the bias values
bias_data <- data.frame(
  Method = c("wp_uc", "wp_c", "gpw", "ghsl"),
  Bias = c(-2908.171, -4076.333, -4188.786, -3056.205)
)

# Create a bar plot
bias<-ggplot(bias_data, aes(x = Method, y = Bias, fill = Method)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(Bias, 1)), vjust = -0.5) +
  theme_minimal() +
  labs(title = "Bias in Population Estimated",
       x = "Estimation from population datasets",
       y = "Bias (Survey Population - Estimated Population)") +
...
```{r}
# Save the plot to a file
ggsave("Plots/bias.png", plot = bias, width = 8, height = 6, dpi = 400)
# Save the plot to a file
...
library(gridExtra)
# Scatter plot with site_status
ms_b_surv<-ggplot(camps, aes(y = as.numeric(ms_b_c), x = N_INDVI, color = Sit_Stt)) +
  #geom_point(alpha=0.2) +
...
# Scatter plot with site_status
g_b_surv<-ggplot(camps, aes(y = as.numeric(g_b_c), x = N_INDVI, color = Sit_Stt)) +
  #geom_point(alpha=0.2) +
 ...
 # Scatter plot with site_status
vida_b_surv<-ggplot(camps, aes(y = as.numeric(vida_b_c), x = N_INDVI, color = Sit_Stt)) +
  #geom_point(alpha=0.2) +
  ...
  # Scatter plot with site_status
osm_b_surv<-ggplot(camps, aes(y = as.numeric(osm_b_c), x = N_INDVI, color = Sit_Stt)) +
  #geom_point(alpha=0.2) +
...
# with slope using geoabline

```{r}
library(ggplot2)
library(gridExtra)

# Scatter plot with site_status and slope of 45 degrees
ms_b_surv <- ggplot(camps, aes(y = as.numeric(ms_b_c), x = as.numeric(N_INDVI), color = Sit_Stt)) +
  geom_jitter(shape = 7, size = 0.5, alpha = 2, show.legend = TRUE) +
  geom_smooth(method = "lm", aes(group = Sit_Stt), se = FALSE) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "blue") +  # Add a slope of 45 degrees
  labs(title = "Comparison between MS building count and survey IOM populations",
       y = "Microsoft building count",
       x = "IOM surveyed population") +
  theme_minimal() +
...
# Scatter plot with site_status and slope of 45 degrees
g_b_surv <- ggplot(camps, aes(y = as.numeric(g_b_c), x = as.numeric(N_INDVI), color = Sit_Stt)) +
  geom_jitter(shape = 7, size = 0.5, alpha = 2, show.legend = TRUE) +
  geom_smooth(method = "lm", aes(group = Sit_Stt), se = FALSE) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "blue") +  # Add a slope of 45 degrees
  labs(title = NULL,
...
# Scatter plot with site_status and slope of 45 degrees
vida_b_surv <- ggplot(camps, aes(y = as.numeric(vida_b_c), x = as.numeric(N_INDVI), color = Sit_Stt)) +
  geom_jitter(shape = 7, size = 0.5, alpha = 2, show.legend = TRUE) +
  geom_smooth(method = "lm", aes(group = Sit_Stt), se = FALSE) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "blue") +  # Add a slope of 45 degrees
  labs(title = NULL,
...
# Scatter plot with site_status and slope of 45 degrees
osm_b_surv <- ggplot(camps, aes(y = as.numeric(osm_b_c), x = as.numeric(N_INDVI), color = Sit_Stt)) +
  geom_jitter(shape = 7, size = 0.5, alpha = 2, show.legend = TRUE) +
  geom_smooth(method = "lm", aes(group = Sit_Stt), se = FALSE) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "blue") +  # Add a slope of 45 degrees
  labs(title = NULL,
...
combined_plot_b <- grid.arrange(ms_b_surv + ylim(0, 10000), g_b_surv + ylim(0, 10000), vida_b_surv + ylim(0, 10000), osm_b_surv + ylim(0, 10000), nrow = 2, ncol = 2)
...
# Combine all scatter plots into one
combined_plot_b_hh <- grid.arrange(ms_b_surv_hh+ylim(0, 10000),g_b_hh+ylim(0, 10000),vida_b_hh+ylim(0, 10000), osm_b_hh+ylim(0, 10000), nrow = 2, ncol = 2)
...
# Scatter plot with site_status and slope of 45 degrees
osm_b_hh <- ggplot(camps, aes(y = osm_b_c, x = N_HH, color = Sit_Stt)) +
  geom_jitter(aes(x = as.numeric(N_HH), y = as.numeric(osm_b_c)), shape = 7, size = 0.5, alpha = 2, show.legend = TRUE) +
  geom_smooth(method = "lm", aes(group = Sit_Stt), se = FALSE) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "blue") +  # Add a slope of 45 degrees
  labs(title = NULL,
...
summarized_data <- data.frame(
  ...
  building_count = c(169994,26279, 67079, 185140)
)
...
bar_plot <- ggplot(summarized_data, aes(x = source, y = building_count, fill = source)) +
  ...
  scale_fill_manual(values = c("Google" = "skyblue", "OSM" = "pink", "Microsoft" = "lightgreen", "VIDA" = "coral")) +
  theme_minimal() +
  theme(legend.position = "none")
...
rsquared_ms_b_c <- corr_ms_b_c^2
rsquared_g_b_c <- corr_g_b_c^2
rsquared_vida_b_c <- corr_vida_b_c^2
rsquared_osm_b_c <- corr_osm_b_c^2
rsquared_ms_b_c_hh <- corr_ms_b_c_hh^2
rsquared_g_b_c_hh <- corr_g_b_c_hh^2
rsquared_vida_b_c_hh <- corr_vida_b_c_hh^2
rsquared_osm_b_c_hh <- corr_osm_b_c_hh^2

# View coefficient of determination (R-squared)
print(rsquared_ms_b_c)
print(rsquared_g_b_c)
print(rsquared_vida_b_c)
print(rsquared_osm_b_c)
print(rsquared_ms_b_c_hh)
print(rsquared_g_b_c_hh)
print(rsquared_vida_b_c_hh)
print(rsquared_osm_b_c_hh)
...
# Assign row and column names
rownames(corr_matrix_b) <- colnames(corr_matrix_b) <- c("Survey Population","Survey Household","Google building count","Microsoft building count","VIDA building count","OSM building count")
# Convert the correlation matrix to long format
...
```
#------correlation with building count-area and nindvi nhh

```{r}
# Calculate correlation coefficients
correlations <- data.frame(
  Metric = rep(c("N_INDVI", "N_HH"), each = 8),
  Variable = c(
    "ms_b_c", "g_b_c", "vida_b_c", "osm_b_c", "ms_b_a", "g_b_a", "vida_b_a", "osm_b_a",
    "ms_b_c", "g_b_c", "vida_b_c", "osm_b_c", "ms_b_a", "g_b_a", "vida_b_a", "osm_b_a"
  ),
  Correlation = c(
    # Correlations with N_INDVI
    cor(camps$N_INDVI, camps$ms_b_c, use = "complete.obs"),
    cor(camps$N_INDVI, camps$g_b_c, use = "complete.obs"),
    cor(camps$N_INDVI, camps$vida_b_c, use = "complete.obs"),
    cor(camps$N_INDVI, camps$osm_b_c, use = "complete.obs"),
    cor(camps$N_INDVI, camps$ms_b_a, use = "complete.obs"),
    cor(camps$N_INDVI, camps$g_b_a, use = "complete.obs"),
    cor(camps$N_INDVI, camps$vida_b_a, use = "complete.obs"),
    cor(camps$N_INDVI, camps$osm_b_a, use = "complete.obs"),
    # Correlations with N_HH
    cor(camps$N_HH, camps$ms_b_c, use = "complete.obs"),
    cor(camps$N_HH, camps$g_b_c, use = "complete.obs"),
    cor(camps$N_HH, camps$vida_b_c, use = "complete.obs"),
    cor(camps$N_HH, camps$osm_b_c, use = "complete.obs"),
    cor(camps$N_HH, camps$ms_b_a, use = "complete.obs"),
    cor(camps$N_HH, camps$g_b_a, use = "complete.obs"),
    cor(camps$N_HH, camps$vida_b_a, use = "complete.obs"),
    cor(camps$N_HH, camps$osm_b_a, use = "complete.obs")
  )
)
...
# Set the rownames manually
rownames(heatmap_data) <- heatmap_data$Metric
heatmap_data <- heatmap_data[, -1]  # Remove the Metric column after setting rownames

# Plot heatmap
library(ggplot2)
library(reshape2)
heatmap_long <- melt(as.matrix(heatmap_data))
...
ggplot(heatmap_long, aes(x = Var2, y = Var1, fill = value)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    x = "Building Data Variables",
    y = "Metrics",
    title = "Correlation Heatmap",
    subtitle = "Correlation of N_INDVI and N_HH with Building Counts and Areas"
  )
...
```{r}
# Calculate correlation coefficients for N_INDVI and N_HH with building counts and areas
correlations <- data.frame(
  Metric = c(
    "ms_b_c", "g_b_c", "vida_b_c", "osm_b_c", "ms_b_a", "g_b_a", "vida_b_a", "osm_b_a"
  ),
...
names(correlations) <- c("Building_Variable", "N_INDVI", "N_HH")

# Reshape the data to long format
library(tidyr)
heatmap_data <- correlations %>%
  pivot_longer(cols = c("N_INDVI", "N_HH"), names_to = "Metric", values_to = "Correlation")

ggplot(heatmap_data, aes(x = Metric, y = Building_Variable, fill = Correlation)) +
  geom_tile() +
  geom_text(aes(label = round(Correlation, 2)), color = "black") +  # Add correlation values
  scale_fill_gradient2(
    low = "blue", high = "red", mid = "white",
    midpoint = 0, limit = c(-1, 1), space = "Lab",
    name = "Correlation"
...
```{r}
# Reorder the levels of Building_Variable and rename them
heatmap_data$Building_Variable <- factor(heatmap_data$Building_Variable,
                                          levels = c("g_b_c","ms_b_c",  "vida_b_c", "osm_b_c", 
                                                      "g_b_a","ms_b_a", "vida_b_a", "osm_b_a"))

# Rename the variables for better clarity
heatmap_data$Building_Variable <- recode(heatmap_data$Building_Variable,
                                          "ms_b_c" = "microsoft_count",
                                          "g_b_c" = "google_count",
                                          "vida_b_c" = "vida_count",
                                          "osm_b_c" = "osm_count",
                                          "ms_b_a" = "microsoft_area",
                                          "g_b_a" = "google_area",
                                          "vida_b_a" = "vida_area",
                                          "osm_b_a" = "osm_area")

# Rename the Metric column
heatmap_data$Metric <- recode(heatmap_data$Metric,
                              "N_INDVI" = "survey_population",
                              "N_HH" = "survey_household")

# Plot heatmap with correlation values
h<-ggplot(heatmap_data, aes(x = Metric, y = Building_Variable, fill = Correlation)) +
  geom_tile() +
  geom_text(aes(label = round(Correlation, 2)), color = "black", size = 4) +  # Add correlation values
  scale_fill_gradient2(
    low = "blue", high = "red", mid = "white",
  ...
  labs(
    ...
    y = "Building Data Variables",
    title = "Correlation Heatmap",
    subtitle = "Correlation of Survey Population and Survey Household with Building Counts and Areas"
  )
print(h)
# Save the plot to a file
ggsave("Plots/building_count_bar.png", plot =h , width = 8, height = 6, dpi = 500)
```
...
# Add R-squared values to the correlation matrix
corr_matrix_long_b$rsquared <- rsquared(corr_matrix_long_b$value)

# Plot the heatmap with lighter color and correlation values
heat_map_corr_b <- ggplot(data = corr_matrix_long_b, aes(x = Var1, y = Var2, fill = rsquared, label = round(rsquared, 2))) +
  geom_tile(color = "white") +
  geom_text(color = "black", size = 2) +
  scale_fill_gradient(low = "lightblue", high = "lightcoral") +
  labs(title = "Correlation Matrix Heatmap with R-squared Values",
       x = "Variables",
       y = "Variables",
       fill = "R-squared") +
  theme_minimal() +
...
```{r}
# Subtract N_HH from the respective columns
camp_final$g_bc_df <- as.numeric(camp_final$N_HH) - as.numeric(camp_final$g_b_c)
camp_final$ms_bc_df <- as.numeric(camp_final$N_HH) - as.numeric(camp_final$ms_b_c)
camp_final$vida_bc_df <- as.numeric(camp_final$N_HH) - as.numeric(camp_final$vida_b_c)
camp_final$osm_bc_df <- as.numeric(camp_final$N_HH) - as.numeric(camp_final$osm_b_c)
...
# Multiply the building differences by -1
difference_data_building$Building_Difference <- -1 * difference_data_building$Building_Difference
...
# Create a box plot
boxplot_build_diff <- ggplot(difference_data_building, aes(x = Dataset_build, y = Building_Difference)) +
  geom_boxplot(fill = "skyblue", color = "black", alpha = 0.7) + # Adjust box transparency
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, color = "red") + # Adding mean points
  geom_text(data = mean_values, aes(label = round(Building_Difference, digits = 2)), vjust = -0.5, size = 3, color = "red") + # Adding mean labels
  labs(title = "Difference between Surveyed Household and Building count (Survey-Estimate) for different Building footprint data products",
       x = "Dataset", y = "Building count Difference") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        plot.title = element_text(size = 10)) + # Rotate x-axis labels
  ylim(-20, 20)  # Adjust the y-axis limits to focus on the area around 0
...
quartiles <- tapply(difference_data_building$Building_Difference, difference_data_building$Dataset_build, quantile)
IQR <- sapply(quartiles, diff)

# Define upper and lower bounds for outliers
outlier_bounds <- sapply(1:4, function(i) c(min(quartiles[[i]]), max(quartiles[[i]]) + 1.5 * IQR[i]))

# Identify outliers
outliers <- Map(function(x, q) x[x < q[1] | x > q[2]], 
                split(difference_data_building$Building_Difference, difference_data_building$Dataset_build), outlier_bounds)

# Create a box plot
boxplot_build_diff <- ggplot(difference_data_building, aes(x = Dataset_build, y = Building_Difference)) +
  geom_boxplot(fill = "skyblue", color = "black", alpha = 0.7) + # Adjust box transparency
  geom_point(data = data.frame(Dataset_build = rep(names(outliers), sapply(outliers, length)),
                               Building_Difference = unlist(outliers)), 
             aes(x = Dataset_build, y = Building_Difference),
             shape = 1, size = 2, alpha = 0.5, color = "red") + # Add outliers separately
  stat_summary(fun = mean, geom = "point", shape = 23, size = 1, color = "red") + # Adding mean points
  labs(title = "Difference between Surveyed Household and Building count (Survey-Estimate) for different Building footprint data products",
       x = "Dataset", y = "Building count Difference") +
  theme_minimal() +
...
#__RATIOS OF ERROR____
#ratio of error for building datasets 
```{r}
# Calculate the ratio of the difference to the surveyed population
...
# Create a box plot for the ratios
boxplot_ratio_build <- ggplot(difference_data_building, aes(x = Dataset_build, y = ratio)) +
  geom_boxplot(fill = "skyblue", color = "black", outlier.alpha = 0.2) + # Adjust outlier transparency
  stat_summary(fun = mean, geom = "point", shape = 23, size = 2, color = "red") + # Adding mean points with larger size
  geom_text(data = mean_ratio_values_build, aes(label = round(ratio, digits = 2)), vjust = -0.5, size = 3, color = "red") + # Adding larger text labels
  labs(title = "Ratio of Difference to Survey Household for Different Building Footprint Data Products",
       x = "Dataset", y = "Ratio of Difference to Survey Household number") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(size = 10),
        plot.margin = margin(1, 1, 1, 1, "cm")) + # Add more margin space around the plot
  coord_cartesian(ylim = c(0, max(difference_data_building$ratio, na.rm = TRUE) * 1.1)) # Adjusting ylim to include some padding
...
# Create a box plot for the ratios
boxplot_ratio_build <- ggplot(difference_data_building, aes(x = Dataset_build, y = ratio)) +
  geom_boxplot(fill = "pink", color = "coral") + # Adjust outlier transparency
  stat_summary(fun = mean, geom = "point", shape = 23, size = 2, color = "blue") + # Adding mean points with larger size
  geom_text(data = mean_ratio_values_build, aes(label = round(ratio, digits = 2)), vjust = -0.5, size = 3, color = "blue") + # Adding larger text labels
  labs(title = "Ratio of Difference to Survey Household for Different Building Footprint Data Products",
       x = "Dataset", y = "Ratio of Difference to Survey Household number") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(size = 10),
        plot.margin = margin(1, 1, 1, 1, "cm")) + # Add more margin space around the plot
  coord_cartesian(ylim = c(-2, 2)) # Adjusting ylim to focus on the main distribution area
...
# Create a box plot for the ratios with reversed y-axis
boxplot_ratio_build <- ggplot(difference_data_building, aes(x = Dataset_build, y = -ratio)) +  # Negative sign to reverse y-axis
  geom_boxplot(fill = "skyblue", color = "black") +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 1, color = "red") +
  geom_text(data = mean_ratio_values_build, aes(label = round(-ratio, digits = 2)), vjust = 0.5, size = 2, color = 'Red') +  # Adjust vjust to position text correctly
  labs(title = "Ratio of Difference to Survey Household for Different Building Footprint Data Products",
       x = "Dataset", y = "Ratio of Difference to Survey Household number") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
# Create a box plot for the ratios with flipped y-axis
boxplot_ratio_build <- ggplot(difference_data_building, aes(x = Dataset_build, y = -ratio)) +  # Flip the y-axis by multiplying ratio by -1
  geom_boxplot(fill = "pink", color = "coral") + # Adjust outlier transparency
  stat_summary(fun = mean, geom = "point", shape = 1, size = 1, color = "black") + # Adding mean points with larger size
  geom_text(data = mean_ratio_values_build, aes(label = round(-ratio, digits = 2)), vjust = 0.5, size = 3, color = "blue") + # Adding larger text labels
  labs(title = "Ratio of Difference to Survey Household for Different Building Footprint Data Products",
       x = "Dataset", y = "Ratio of Difference to Survey Household number") +
  theme_minimal() +
...
# HOUSEHOLD/BUIDING COUNT

```{r}
# Ensure necessary libraries are loaded
library(dplyr)

# Create a new data frame to hold the number of households per building
households_per_building <- camps %>%
  mutate(
   hh_per_building_ms = ifelse(ms_b_c == 0, NA, N_HH / ms_b_c),
    hh_per_building_osm = ifelse(osm_b_c == 0, NA, N_HH / osm_b_c),
    hh_per_building_google = ifelse(g_b_c == 0, NA, N_HH / g_b_c),
    hh_per_building_vida = ifelse(vida_b_c == 0, NA, N_HH / vida_b_c)
  )

# View the calculated data
head(households_per_building)

# Reshape the data for visualization
households_per_building_long <- households_per_building %>%
  select(Name, N_HH, hh_per_building_ms, hh_per_building_osm, hh_per_building_google, hh_per_building_vida) %>%
  gather(key = "Dataset", value = "HH_per_Building", hh_per_building_ms:hh_per_building_vida)

# View the reshaped data
head(households_per_building_long)
...
# Create the bar plot
ggplot(households_per_building_long, aes(x = Dataset, y = HH_per_Building, fill = Dataset)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Number of Households per Building by Dataset",
       x = "Dataset",
       y = "Households per Building") +
  scale_fill_brewer(palette = "Set2") +
  theme(legend.position = "none")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Calculate households per building area
households_per_building_area <- camps %>%
  mutate(
    hh_per_area_ms = ifelse(ms_b_a == 0, NA, N_HH / ms_b_a),
    hh_per_area_osm = ifelse(osm_b_a == 0, NA, N_HH / osm_b_a),
    hh_per_area_google = ifelse(g_b_a == 0, NA, N_HH / g_b_a),
    hh_per_area_vida = ifelse(vida_b_a == 0, NA, N_HH / vida_b_a)
  )

# Convert data to long format for ggplot2
households_per_building_area_long <- households_per_building_area %>%
  select(Name, hh_per_area_ms, hh_per_area_osm, hh_per_area_google, hh_per_area_vida) %>%
  pivot_longer(cols = starts_with("hh_per_area"), 
               names_to = "Dataset", 
               values_to = "Households_per_Area")

# Create box plot with slanting x-axis labels
hh_per_b_a<-ggplot(households_per_building_area_long, aes(x = Dataset, y = Households_per_Area)) +
  geom_boxplot() +
  theme_minimal() +
  labs(
    title = "Number of Households per Building Area m²",
    x = "Dataset",
    y = "Households per Building Area m²"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
print(hh_per_b_a)
...
```{r}
# Calculate square meters per household and per person
sqm_metrics <- camps %>%
  mutate(
    # Square meters per household
    sqm_per_hh_ms = ifelse(ms_b_a == 0, NA, ms_b_a / N_HH),
    sqm_per_hh_osm = ifelse(osm_b_a == 0, NA, osm_b_a / N_HH),
    sqm_per_hh_google = ifelse(g_b_a == 0, NA, g_b_a / N_HH),
    sqm_per_hh_vida = ifelse(vida_b_a == 0, NA, vida_b_a / N_HH),
    
    # Square meters per person
    sqm_per_person_ms = ifelse(ms_b_a == 0, NA, ms_b_a / N_INDVI),
    sqm_per_person_osm = ifelse(osm_b_a == 0, NA, osm_b_a / N_INDVI),
    sqm_per_person_google = ifelse(g_b_a == 0, NA, g_b_a / N_INDVI),
    sqm_per_person_vida = ifelse(vida_b_a == 0, NA, vida_b_a / N_INDVI)
  )

# Convert to long format for visualization
sqm_long <- sqm_metrics %>%
  select(Name, 
         starts_with("sqm_per_hh"), 
         starts_with("sqm_per_person")) %>%
  pivot_longer(
    cols = starts_with("sqm_"), 
    names_to = "Metric", 
    values_to = "Value"
  ) %>%
  separate(Metric, into = c("Metric", "Dataset"), sep = "_per_")

# Create separate plots for households and individuals
# Square meters per household
sqm_hh_plot <- ggplot(
  filter(sqm_long, Metric == "sqm"), 
  aes(x = Dataset, y = Value, fill = Dataset)
) +
  geom_boxplot() +
  theme_minimal() +
  labs(
    title = "Square Meters per Household",
    x = "Dataset",
    y = "Square Meters per Household"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Square meters per person
sqm_person_plot <- ggplot(
  filter(sqm_long, Metric == "sqm" & Dataset == "person"), 
  aes(x = Dataset, y = Value, fill = Dataset)
) +
  geom_boxplot() +
  theme_minimal() +
  labs(
    title = "Square Meters per Person",
    x = "Dataset",
    y = "Square Meters per Person"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

print(sqm_person_plot)
```


#checking fot the NA values
...
camps_nonspatial <- camps %>%
  st_drop_geometry() %>%
  select(Name, N_HH, ms_b_a, osm_b_a, g_b_a, vida_b_a)

# Calculate household density metrics
households_per_building_area <- camps_nonspatial %>%
  mutate(
    hh_per_area_ms = ifelse(ms_b_a == 0, NA, N_HH / ms_b_a),
    hh_per_area_osm = ifelse(osm_b_a == 0, NA, N_HH / osm_b_a),
    hh_per_area_google = ifelse(g_b_a == 0, NA, N_HH / g_b_a),
...
# Count NA values for each dataset
na_counts <- households_per_building_area %>%
  summarise(
    hh_per_area_ms_na = sum(is.na(hh_per_area_ms)),
    hh_per_area_osm_na = sum(is.na(hh_per_area_osm)),
    hh_per_area_google_na = sum(is.na(hh_per_area_google)),
    hh_per_area_vida_na = sum(is.na(hh_per_area_vida))
  ) %>%
  pivot_longer(cols = everything(), names_to = "Dataset", values_to = "NA_Count")
# Visualize NA counts using a bar plot
na_counts_plot <- ggplot(na_counts, aes(x = Dataset, y = NA_Count, fill = Dataset)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(
    title = "Count of NA Values in Household Density Data",
    x = "Dataset",
...
#save
```{r}
ggsave("Plots/hh_per_b_a.png", plot =hh_per_b_a , width = 8, height = 6, dpi = 400)
# Save the plot to a file
ggsave("Plots/na_counts_plot.png", plot =na_counts_plot , width = 8, height = 6, dpi = 400)
```
#thesis used
#---Population per building area and household/building area of the building footprint dataset
```{r}
library(dplyr)
library(tidyr)
library(ggplot2)
...
# Calculate households and population per building area
households_and_population_per_building_area <- camps %>%
  mutate(
    hh_per_area_ms = ifelse(ms_b_a == 0, NA, N_HH / ms_b_a),
    hh_per_area_osm = ifelse(osm_b_a == 0, NA, N_HH / osm_b_a),
    hh_per_area_google = ifelse(g_b_a == 0, NA, N_HH / g_b_a),
    hh_per_area_vida = ifelse(vida_b_a == 0, NA, N_HH / vida_b_a),
    pop_per_area_ms = ifelse(ms_b_a == 0, NA, N_INDVI / ms_b_a),
    pop_per_area_osm = ifelse(osm_b_a == 0, NA, N_INDVI / osm_b_a),
    pop_per_area_google = ifelse(g_b_a == 0, NA, N_INDVI / g_b_a),
    pop_per_area_vida = ifelse(vida_b_a == 0, NA, N_INDVI / vida_b_a),
    osm_no_buildings = ifelse(is.na(osm_b_a) | osm_b_a == 0, 1, 0)
  )

# Convert data to long format for ggplot2
households_per_building_area_long <- households_and_population_per_building_area %>%
  select(Name, hh_per_area_ms, hh_per_area_osm, hh_per_area_google, hh_per_area_vida) %>%
  pivot_longer(cols = starts_with("hh_per_area"), 
               names_to = "Dataset", 
               values_to = "Households_per_Area")

population_per_building_area_long <- households_and_population_per_building_area %>%
  select(Name, pop_per_area_ms, pop_per_area_osm, pop_per_area_google, pop_per_area_vida) %>%
  pivot_longer(cols = starts_with("pop_per_area"), 
               names_to = "Dataset", 
               values_to = "Population_per_Area")

# Create box plots
households_plot <- ggplot(households_per_building_area_long, aes(x = Dataset, y = Households_per_Area)) +
  geom_boxplot() +
  theme_minimal() +
  labs(
    title = "Number of Households per Building Area (m²)",
...
population_plot <- ggplot(population_per_building_area_long, aes(x = Dataset, y = Population_per_Area)) +
  geom_boxplot() +
  theme_minimal() +
  labs(
    title = "Number of Individuals per Building Area (m²)",
    x = "Dataset",
    y = "Individuals per Building Area (m²)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Arrange plots using grid.arrange
pop_hh_per_ba<-grid.arrange(households_plot, population_plot, ncol = 2)
print(pop_hh_per_ba)
...
# Example of spatial analysis for building footprint count based on representative datasets
# Assuming the datasets have columns `Name`, `ms_b_a`, `osm_b_a`, `g_b_a`, `vida_b_a` (building areas) and `N_HH` (number of households)

# Create a summarized dataset for analysis
building_footprint_analysis <- camps %>%
  summarize(
    count_google = sum(!is.na(g_b_a) & g_b_a > 0),
    count_ms = sum(!is.na(ms_b_a) & ms_b_a > 0),
    count_osm = sum(!is.na(osm_b_a) & osm_b_a > 0),
    count_vida = sum(!is.na(vida_b_a) & vida_b_a > 0),
    total_google_area = sum(g_b_a, na.rm = TRUE),
    total_ms_area = sum(ms_b_a, na.rm = TRUE),
    total_osm_area = sum(osm_b_a, na.rm = TRUE),
    total_vida_area = sum(vida_b_a, na.rm = TRUE)
  )

# Add a long-format table for box plot plotting
households_and_population_per_building_area <- camps %>%
  mutate(
    hh_per_area_ms = ifelse(ms_b_a == 0, NA, N_HH / ms_b_a),
    hh_per_area_osm = ifelse(osm_b_a == 0, NA, N_HH / osm_b_a),
...
# Customize Dataset labels for plotting
households_per_building_area_long$Dataset <- factor(households_per_building_area_long$Dataset, 
                                                    levels = c("hh_per_area_google", "hh_per_area_ms", "hh_per_area_osm", "hh_per_area_vida"), 
                                                    labels = c("Google", "Microsoft", "OSM", "VIDA"))

population_per_building_area_long$Dataset <- factor(population_per_building_area_long$Dataset, 
                                                    levels = c("pop_per_area_google", "pop_per_area_ms", "pop_per_area_osm", "pop_per_area_vida"), 
                                                    labels = c("Google", "Microsoft", "OSM", "VIDA"))

# Create box plots with customized colors
households_plot <- ggplot(households_per_building_area_long, aes(x = Dataset, y = Households_per_Area, fill = Dataset)) +
  geom_boxplot(outlier.alpha = 0.2) +
  stat_summary(fun = median, geom = "text", aes(label = round(..y.., 2)), vjust = -0.5, color = "red", size = 3) +
  scale_fill_manual(values = c("#D1E5F0", "#FEE5D9", "#CCEBC5", "#FFFFCC")) +
  theme_minimal() +
  labs(
    title = "Number of Households per Building Area (m²)",
    x = "Dataset",
...
population_plot <- ggplot(population_per_building_area_long, aes(x = Dataset, y = Population_per_Area, fill = Dataset)) +
  geom_boxplot(outlier.alpha = 0.2) +
  stat_summary(fun = median, geom = "text", aes(label = round(..y.., 2)), vjust = -0.5, color = "red", size = 3) +
  scale_fill_manual(values = c("#D1E5F0", "#FEE5D9", "#CCEBC5", "#FFFFCC")) +
  theme_minimal() +
...
```{r}
# Save the plot to a file
ggsave("Plots/pop_hh_per_ba.png", plot =pop_hh_per_ba , width = 8, height = 6, dpi = 400)
```
...
box_dou_indv<-ggplot(camp_final, aes(x = dou_level1, y = N_INDVI)) +
  ...
  stat_summary(fun = median, geom = "text", aes(label = round(..y.., 2)), vjust = -0.5, show.legend = FALSE) +  # Add mean text
...
p <- ggplot(camp_final_sum, aes(x = Source, y = Building_Area, fill = dou_level1)) +
  ...
  scale_fill_manual(values = c( "blue", "lightgreen", "coral"),
                    name = "Dou Level 1") +
...
buildings_area<-p + geom_text(aes(label = format(Building_Area, big.mark = ",", scientific = FALSE)), position = position_stack(vjust = 0.5), size = 2.5, color = "black")
print(buildings_area)
```

```{r}
# Save the plot to a file
ggsave("Plots/dou_1_building_area.png", plot =buildings_area, width = 8, height = 6, dpi = 400)
```

#dou_level and population density
```{r}
library(ggplot2)

# Convert `st_pp_d` and `dou_level1` columns to appropriate types if necessary
camp_final$st_pp_d <- as.numeric(camp_final$st_pp_d)
camp_final$dou_level1 <- as.factor(camp_final$dou_level1)

# Calculate mean values
mean_values <- aggregate(st_pp_d ~ dou_level1, data = camp_final, mean)

# Create a box plot for st_pp_d grouped by dou_level1
boxplot_st_pp_d <- ggplot(camp_final, aes(x = dou_level1, y = st_pp_d, fill = dou_level1)) +
  geom_boxplot() +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, color = "red") + # Adding mean points
  geom_text(data = mean_values, aes(label = round(st_pp_d, digits = 2), y = st_pp_d), 
            vjust = -0.5, size = 3, color = "blue") + # Adding mean text labels
  geom_jitter(width = 0.2, size = 1, color = "black", alpha = 0.5) + # Adding jitter for better visibility of individual points
  labs(title = "Distribution of population density (sq m)by dou_level1",
       x = "Dou Level 1",
       y = "st_pp_d",
       fill = "Dou Level 1") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  coord_cartesian(ylim = c(-1, max(camp_final$st_pp_d, na.rm = TRUE))) # Adjusting y-axis limits to start from -1

# Print the box plot
print(boxplot_st_pp_d)
...
ggsave("D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/boxplot_st_pp_d.png", plot = boxplot_st_pp_d, width = 8, height = 6, dpi = 300)
```
...
camp_final$N_INDVI <- as.numeric(camp_final$N_INDVI)
camp_final$st_pp_d <- as.numeric(camp_final$st_pp_d)

# Create the scatter plot with a linear regression line and a 45-degree slope line
scatter_plot <- ggplot(camp_final, aes(x = N_INDVI, y = st_pp_d, color = dou_level1)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +  # Linear regression line
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "blue") +  # 45-degree slope line
  scale_y_continuous(limits = c(-1, max(camp_final$st_pp_d, na.rm = TRUE))) +  # Adjust y-axis to start from a negative value
  labs(title = "Scatter Plot of Site Population Density vs N_INDVI",
       x = "N_INDVI",
       y = "Site Population Density (st_pp_d)") +
  theme_minimal()
...
#calcualte population density with the calculated shape area 
```{r}
# Calculate population density

camp_final$cl_Shp_a <- as.numeric(camp_final$cl_Shp_a)
camp_final$N_INDVI <- as.numeric(camp_final$N_INDVI)
camp_final$pp_d_cl_a <- (camp_final$N_INDVI) / (camp_final$cl_Shp_a)

# Print the result
# Print or view the selected camps
print(camp_final[, c("Name", "N_INDVI", "Site_Ar","pp_d_cl_a")])

```

#population density from calculated area and dou level
```{r}
library(ggplot2)

# Convert `st_pp_d` and `dou_level1` columns to appropriate types if necessary
camp_final$pp_d_cl_a <- as.numeric(camp_final$pp_d_cl_a)
camp_final$dou_level1 <- as.factor(camp_final$dou_level1)

# Calculate mean values
mean_values <- aggregate(pp_d_cl_a~ dou_level1, data = camp_final, mean)

# Create a box plot for st_pp_d grouped by dou_level1
boxplot_pp_d_cl_a <- ggplot(camp_final, aes(x = dou_level1, y = pp_d_cl_a, fill = dou_level1)) +
  geom_boxplot() +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, color = "red") + # Adding mean points
  geom_text(data = mean_values, aes(label = round(pp_d_cl_a, digits = 2), y = pp_d_cl_a), 
            vjust = -0.5, size = 3, color = "blue") + # Adding mean text labels
  geom_jitter(width = 0.2, size = 1, color = "black", alpha = 0.5) + # Adding jitter for better visibility of individual points
  labs(title = "Distribution of population density (sq m)by dou_level1",
       x = "Dou Level 1",
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  coord_cartesian(ylim = c(-1, max(camp_final$pp_d_cl_a, na.rm = TRUE))) # Adjusting y-axis limits to start from -1

# Print the box plot
print(boxplot_pp_d_cl_a)
```
```{r}
# Save the plot as an image file
ggsave("D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/boxplot_pp_d_cl_a.png", plot = boxplot_pp_d_cl_a, width = 8, height = 6, dpi = 300)
```


# Box plot with estimated pop density from cl shape area for dou level 1
```{r}
# Load necessary libraries
library(dplyr)
library(ggplot2)

# Assuming `camp_final` is your dataset containing relevant columns including `cl_Shp_a`, `ghsl`, `gpw_UN`, `wp_c`, `wp_uc`, and `dou_level1`
# Calculate population density for each variable
camp_final <- camp_final %>%
  mutate(
   den_ghsl= ghsl / cl_Shp_a,
   den_gpwu= gpw_UN / cl_Shp_a,
    den_wpc = wp_c / cl_Shp_a,
    den_wpuc = wp_uc / cl_Shp_a
  )

# Create a long format of the dataset for plotting
camp_final_long <- camp_final %>%
  select(dou_level1,  den_ghsl, den_gpwu, den_wpc, den_wpuc) %>%
  pivot_longer(cols = starts_with("den"),
               names_to = "Dataset", 
               values_to = "Population_Density")

# Create a box plot for the population density estimates grouped by dou_level1
boxplot_density <- ggplot(camp_final_long, aes(x = dou_level1, y = Population_Density, fill = Dataset)) +
  geom_boxplot() +
  labs(title = "Population Density Estimates by Dataset and dou_level1",
       x = "dou_level1",
...
# Create a box plot for the population density estimates grouped by Dataset and colored by dou_level1
boxplot_density <- ggplot(camp_final_long, aes(x = Dataset, y = Population_Density, fill = dou_level1)) +
  geom_boxplot() +
  labs(title = "Population Density Estimates by Dataset and dou_level1",
       x = "Dataset",
       y = "Population Density",
...
```{r}
# Save the plot as an image file
ggsave("D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/boxplot_density.png", plot = boxplot_density, width = 8, height = 6, dpi = 300)
```
...
# Create a long format of the dataset for plotting
camps_long <- camps %>%
  select(g_b_c, g_c_65_to_, g_c_70_to_, g_c_75_to_) %>%
  pivot_longer(cols = starts_with("g_c_"),
               names_to = "Group", 
               values_to = "Count")

# Summarize the data by calculating the mean of g_b_c for each Group and Count
summary_data <- camps_long %>%
  group_by(Group, Count) %>%
  summarize(mean_g_b_c = mean(g_b_c, na.rm = TRUE))

# Create bar plots for each group
bar_plot <- ggplot(summary_data, aes(x = Count, y = mean_g_b_c, fill = Group)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ Group, scales = "free_x") +
  labs(title = "Mean g_b_c by Group Counts",
       x = "Count",
       y = "Mean g_b_c",
       fill = "Group") +
  theme_minimal() +
...
# Save the plot as an image file
ggsave("bar_plot_relationship.png", plot = bar_plot, width = 10, height = 6, dpi = 300)
...
```{r}
lm_model <- lm(st_pp_d ~ N_INDVI + N_HH + g_b_c + ms_b_c + vida_b_c + osm_b_c + dou_level1, data = camp_final)
summary(lm_model)

```
```{r}
multi_lm_model <- lm(st_pp_d ~ N_INDVI * dou_level1 + N_HH * dou_level1, data = camp_final)
summary(multi_lm_model)

```

```{r}
chisq_test <- chisq.test(table(camp_final$dou_level1, camp_final$Site_Ar))
chisq_test
...
```{r}
library(cluster)
kmeans_model <- kmeans(camp_final[, numeric_cols], centers = 3)
camp_final$cluster <- as.factor(kmeans_model$cluster)

ggplot(camp_final, aes(x = N_INDVI, y = st_pp_d, color = cluster)) +
  geom_point() +
  labs(title = "K-Means Clustering of Site Population Density", 
       x = "N_INDVI", y = "Site Population Density (st_pp_d)") +
  theme_minimal()
...
```
#----
#correcting the wp_uc difference issue

```{r}
camp_final$wp_c_df<- as.numeric(camps$N_INDVI) - as.numeric(camp_final$wp_c)
camp_final$wp_uc_df<- as.numeric(camps$N_INDVI) - as.numeric(camp_final$wp_uc)
print(camp_final$wp_c_df) 
print(camp_final$wp_uc_df)
...
#thesis used
#Error ratio based on the degree of urbanisation
```{r}
...
difference_data_dou <- data.frame(
  Dataset = rep(c( "GPW-v4","GHS-POP","WorldPop_unconstrained","WorldPop_constrained"), each = nrow(camp_final)),
  Population_Difference_dou = c(as.numeric(camp_final$gpwU_df), as.numeric(camp_final$ghsl_df), as.numeric(camp_final$wp_uc_df),as.numeric(camp_final$wp_c_df)),
  dou_level1 = camp_final$dou_level1  # Adding dou_level1 column
)

# Ensure the correct order of the factors for the Dataset variable
difference_data_dou$Dataset <- factor(difference_data_dou$Dataset, levels = c("GPW-v4","GHS-POP","WorldPop_unconstrained","WorldPop_constrained"))
...
boxplot_ratio <- ggplot(difference_data_dou, aes(x = Dataset, y = -ratio, fill = dou_level1)) +
  ...
  stat_summary(fun = median, geom = "point", shape = 23, size = 0.5, color = "green") +
 stat_summary(fun = median, geom = "text", aes(label = round(..y.., digits = 2)), 
               vjust = -0.5, size = 3, color = 'blue') +stat_summary(
    fun = median, 
    geom = "text", 
    aes(label = round(..y.., digits = 2), group = dou_level1), 
    position = position_dodge(width = 0.75),  # Use position_dodge to separate the text labels
    vjust = -0.5, size = 3, color = "black") +
...
#population box focus and flipping negative coord




```{r}
# Create a box plot for the ratios grouped by dou_level with y-axis multiplied by -1 and adjusted y-axis limits
boxplot_ratio <- ggplot(difference_data_dou, aes(x = Dataset, y = -ratio, fill = dou_level1)) +
  geom_boxplot(width = 0.6, outlier.alpha = 0.2) +  # Adjust the width and lightening the outliers here
  stat_summary(fun = median, geom = "point", shape = 23, size = 0.3, color = "blue") +  # Changing the color of the mean
  stat_summary(
    fun = median, 
    geom = "text", 
    aes(label = round(..y.., digits = 2), group = dou_level1), 
    position = position_dodge(width = 0.75),  # Use position_dodge to separate the text labels
    vjust = -0.5, size = 3, color = "black") +
  labs(title = "Ratio of Difference to Survey Population by EO Population Data Products and dou_level",       x = "Dataset", y = "Ratio of Difference to Survey Population", fill = "dou_level") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5),
        plot.title = element_text(size = 10)) +
  coord_cartesian(ylim = c(-5, 5))+
  scale_x_discrete(labels = function(x) str_wrap(x, width = 7)) # Wrap x-axis labels  # Adjusting y-axis limits to focus more on the box region
...
```{r}
boxplot_ratio <- ggplot(difference_data_dou, aes(x = Dataset, y = -ratio, fill = dou_level1)) +
  geom_boxplot(width = 0.6, outlier.alpha = 0.2) +  # Adjust the width and lightening the outliers
  stat_summary(fun = median, geom = "point", shape = 23, size = 0.3, color = "blue") +  # Median as point
  stat_summary(
    fun = median, 
    geom = "text", 
    aes(label = round(..y.., digits = 2), group = dou_level1), 
    position = position_dodge(width = 0.75),  # Use position_dodge to separate the text labels
    vjust = -0.5, size = 3, color = "black") +  # Adding the median value as text for each dou_level1
  labs(
    #title = "Ratio of Difference to Survey Population by EO Population Data Products and dou_level",
    x = "Dataset", 
    y = "Ratio of Difference to Survey Population", 
    fill = "dou_level"
  ) +
  ...
  coord_cartesian(ylim = c(-10, 10)) +  # Adjust y-axis to focus on the middle region, excluding extreme values
  scale_x_discrete(labels = function(x) str_wrap(x, width = 7))  # Wrap x-axis labels
...
```{r}
boxplot_ratio <- ggplot(difference_data_dou, aes(x = Dataset, y = -ratio, fill = dou_level1)) +
  geom_boxplot(width = 0.6, outlier.alpha = 0.2) + # Adjust the width and lighten the outliers
  stat_summary(fun = median, geom = "point", shape = 23, size = 0.3, color = "blue") +  # Median as point
  stat_summary(
    fun = median, 
    geom = "text", 
  ...
  labs(
    x = "Dataset", 
    y = "Ratio of Difference to Survey Population", 
    fill = "Dou Level"  # Update legend title
  ) +
  theme_minimal() +
  theme(
  ...
  coord_cartesian(ylim = c(-10, 10)) +  # Adjust y-axis to focus on the middle region
  scale_x_discrete(labels = function(x) str_wrap(x, width = 7)) +  # Wrap x-axis labels
  scale_fill_manual(
    values = c(
      "Rural" = "#A6D854", 
      "Urban Centre" = "#E2007A", 
      "Urban Cluster" = "#4169E1"
    )
  )  # Assign specific colors to dou_level1 categories
...
#interactive pop diff ratio with dou
```{r}
library(plotly)
...
# Multiply the ratio by -1
difference_data_dou$ratio <- -difference_data_dou$ratio

# Aggregate mean ratio values
mean_ratio_values <- aggregate(ratio ~ Dataset + dou_level1, data = difference_data_dou, mean)

# Create an interactive box plot for the ratios
boxplot_ratio_interactive <- plot_ly(difference_data_dou, x = ~Dataset, y = ~ratio, color = ~dou_level1, type = "box") %>%
  add_markers(text = ~paste("Dataset:", Dataset, "<br>Name:", camp_final$Name, "<br>Sit_Stt:", camp_final$Sit_Stt, "<br>cl_Shp_a:", camp_final$cl_Shp_a, "<br>Ratio:", ratio), showlegend = FALSE) %>%
  add_markers(data = mean_ratio_values, x = ~Dataset, y = ~ratio, mode = "markers", marker = list(color = "blue", symbol = "diamond", size = 10), 
              text = ~paste("Mean Ratio:", round(ratio, digits = 2)), showlegend = FALSE) %>%
  layout(title = "Ratio of Difference to Survey Population by EO Population Data Products and dou_level",
         xaxis = list(title = "Dataset"),
         yaxis = list(title = "Ratio of Difference to Survey Population", range = c(-10, 10)),
         legend = list(title = list(text = "dou_level")),
         barmode = "overlay")
...
#building error ratio by urban rural 


```{r}
# Combine all population difference columns into one data frame for building dataset
difference_data_building <- data.frame(
  Dataset_build = rep(c("GOOGLE", "OSM", "MICROSOFT", "VIDA"), each = nrow(camp_final)),
  Building_Difference = c(as.numeric(camp_final$g_bc_df), as.numeric(camp_final$osm_bc_df), as.numeric(camp_final$ms_bc_df), as.numeric(camp_final$vida_bc_df)),
  dou_level1 = rep(camp_final$dou_level1, 4)  # Adding dou_level1 column
)


# Calculate the ratio of the difference to the surveyed population
difference_data_building$ratio <- -1 * (difference_data_building$Building_Difference / camps$N_HH)

# Aggregate mean ratio values
mean_ratio_values_build <- aggregate(ratio ~ Dataset_build + dou_level1, data = difference_data_building, mean)

# Create a box plot for the ratios grouped by dou_level
boxplot_ratio_build <- ggplot(difference_data_building, aes(x = Dataset_build, y = ratio, fill = dou_level1)) +
  geom_boxplot(width = 0.6,outlier.colour = "lightgrey") +
  stat_summary(fun = mean, geom = "point", shape = 23, size = .5, color = "black") +
  geom_text(data = mean_ratio_values_build, aes(label = round(ratio, digits = 2), group = dou_level1), 
            position = position_dodge(width = 0.75), vjust = -0.5, size = 3, color = "blue") +
  labs(title = "Ratio of Difference to Survey Household for Different Building Footprint Data Products",
       x = "Dataset", y = "Ratio of Difference to Survey Household number", fill = "dou_level") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(size = 10)) +
  coord_cartesian(ylim = c(-10, 10))  # Adjusting y-axis limits to focus more on the box region
...
```{r}
# Save the plot as an image file
ggsave("D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/boxplot_ratio_error_build.png", plot = boxplot_ratio_build, width = 8, height = 6, dpi = 300)
```
...
# Convert camp_final to an sf object if it is not already

camp_final_sf <- st_as_sf(camp_final)
camps_wp_uc_sf<-st_as_sf(camps_wp_uc)
# Define the file path for saving
output_directory <- "D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/"  
output_file <- paste0(output_directory, "camp_uncons.shp")

# Save the shapefile
st_write(camps_wp_uc_sf, dsn = output_directory, layer = "camp_uncons", driver = "ESRI Shapefile", delete_dsn = TRUE)
...
# Define the file path for saving
output_file <- "D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/camps.xlsx"

# Save the data as an Excel file
write_xlsx(camps, output_file)
...
```{r}
# Ensure camp_final is an sf object
if (!inherits(camps, "sf")) {
  campssf <- st_as_sf(camps)
} else {
  campssf <- camps
}
...
output_directory <- "D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/"
output_file <- paste0(output_directory, "camps.shp")

# Save the shapefile
st_write(campssf, dsn = output_directory, layer = "camps", driver = "ESRI Shapefile", delete_dsn = TRUE)

```

```{r}
library(rgdal)
writeOGR(campssf, dsn = "C:/temp/", layer = "camps", driver = "ESRI Shapefile")
...
# Step 1: Calculate adjusted GPW density (per m²) for 1 km resolution
gpw_density_m2 <- camps$gpw / 1e6  # Convert GPW density from per km² to per m²

# Step 2: Calculate settlement product areas
camps$gpw_area <- camps$gpw / gpw_density_m2  # GPW area (adjusted for resolution)
camps$ghsl_area <- camps$ghsl / camps$st_pp_d  # GHS-POP area
camps$wp_c_area <- camps$wp_c / camps$st_pp_d  # WorldPop constrained area
camps$wp_uc_area <- camps$wp_uc / camps$st_pp_d  # WorldPop unconstrained area

# Step 3: Calculate ratios of settlement product area to surveyed camp area
camps$gpw_ratio <- camps$gpw_area / camps$Site_Ar
camps$ghsl_ratio <- camps$ghsl_area / camps$Site_Ar
camps$wp_c_ratio <- camps$wp_c_area / camps$Site_Ar
camps$wp_uc_ratio <- camps$wp_uc_area / camps$Site_Ar

# Step 4: Prepare data for visualization
coverage_data <- data.frame(
  Dataset = rep(c("GPW-v4", "GHS-POP", "WorldPop_constrained", "WorldPop_unconstrained"), each = nrow(camps)),
  Coverage_Ratio = c(camps$gpw_ratio, camps$ghsl_ratio, camps$wp_c_ratio, camps$wp_uc_ratio),
  Site_Status = camps$Sit_Stt  # Formal/Informal site status
)

# Ensure Dataset is a factor with the correct order
coverage_data$Dataset <- factor(coverage_data$Dataset, levels = c("GPW-v4", "GHS-POP", "WorldPop_constrained", "WorldPop_unconstrained"))

# Step 5: Visualization - Boxplot of coverage ratios
coverage_plot <- ggplot(coverage_data, aes(x = Dataset, y = Coverage_Ratio, fill = Site_Status)) +
  geom_boxplot(color = "black", outlier.color = "gray") +
  stat_summary(fun = median, geom = "point", shape = 23, size = 2, color = "red", fill = "yellow") +
  stat_summary(fun = median, geom = "text", aes(label = round(..y.., 2)), vjust = -0.5, size = 3, color = "blue") +
  scale_fill_manual(values = c("Formal" = "#76c893", "Informal" = "#f4a582")) +  # Custom colors for site status
  labs(
    title = "Coverage Ratios of Population Datasets for Refugee Settlements",
    x = "Dataset",
    y = "Coverage Ratio (Settlement Area / Surveyed Area)",
    fill = "Site Status"
  ) +
  theme_minimal() +
  theme(
  ...
  coord_cartesian(ylim = c(0, 5))  # Adjust y-axis range if needed
...
camps$gpw_percentage <- (camps$gpw_area / camps$Site_Ar) * 100
camps$ghsl_percentage <- (camps$ghsl_area / camps$Site_Ar) * 100
camps$wp_c_percentage <- (camps$wp_c_area / camps$Site_Ar) * 100
camps$wp_uc_percentage <- (camps$wp_uc_area / camps$Site_Ar) * 100

# Step 4: Prepare data for visualization
coverage_data <- data.frame(
  Dataset = rep(c("GPW-v4", "GHS-POP", "WorldPop_constrained", "WorldPop_unconstrained"), each = nrow(camps)),
  Coverage_Percentage = c(camps$gpw_percentage, camps$ghsl_percentage, camps$wp_c_percentage, camps$wp_uc_percentage),
  Site_Status = camps$Sit_Stt  # Formal/Informal site status
)
...
coverage_violin_plot <- ggplot(coverage_data, aes(x = Dataset, y = Coverage_Percentage, fill = Site_Status)) +
  geom_violin(trim = FALSE, alpha = 0.7) +
  stat_summary(fun = median, geom = "point", shape = 23, size = 2, color = "black", fill = "yellow") +
  stat_summary(fun = median, geom = "text", aes(label = round(..y.., 1)), vjust = -0.5, size = 3, color = "blue") +
  scale_fill_manual(values = c("Formal" = "#76c893", "Informal" = "#f4a582")) +  # Custom colors for site status
  labs(
    title = "Population Product Coverage (%) of Refugee Settlement Boundary Area",
    x = "Dataset",
  ...
  coord_cartesian(ylim = c(0, 200))  # Adjust y-axis range if needed

# Print the violin plot
print(coverage_violin_plot)

```

```{r}
# Save the plot as an image file
ggsave("D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/coverage_violin_plot.png", plot = coverage_violin_plot, width = 8, height = 6, dpi = 300)
```
...
coverage_violin_plot <- ggplot(coverage_data, aes(x = Dataset, y = Coverage_Percentage, fill = Site_Status)) +
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, size = 10),  # Horizontal x-axis labels
    plot.title = element_text(size = 12, face = "bold"),
    legend.position = "top"  # Position the legend at the top
  ) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +  # Wrap x-axis labels
  coord_cartesian(ylim = c(-10, 200))  # Extend y-axis to negative region
...
# Step 5: Visualization - Box plot with separate median for Formal and Informal
coverage_boxplot <- ggplot(coverage_data, aes(x = Dataset, y = Coverage_Percentage, fill = Site_Status)) +
  geom_boxplot(alpha = 0.7, outlier.size = 2, outlier.colour = "red") +  # Box plot with outliers
  stat_summary(data = subset(coverage_data, Site_Status == "Formal"), fun = median, geom = "point", shape = 23, size = 3, color = "blue", fill = "yellow") +  # Median point for Formal
  stat_summary(data = subset(coverage_data, Site_Status == "Informal"), fun = median, geom = "point", shape = 23, size = 3, color = "blue", fill = "yellow") +  # Median point for Informal
  stat_summary(data = subset(coverage_data, Site_Status == "Formal"), fun = median, geom = "text", aes(label = round(..y.., 1)), vjust = -1.5, size = 3, color = "blue") +  # Median label for Formal
  stat_summary(data = subset(coverage_data, Site_Status == "Informal"), fun = median, geom = "text", aes(label = round(..y.., 1)), vjust = -1.5, size = 3, color = "blue") +  # Median label for Informal
  scale_fill_manual(values = c("Formal" = "#76c893", "Informal" = "#f4a582")) +  # Custom colors for site status
  labs(
    title = "Population Product Coverage (%) of Refugee Settlement Boundary Area",
    x = "Dataset",
  ...
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +  # Wrap x-axis labels for readability
  coord_cartesian(ylim = c(-2, 300))  # Extend y-axis to the range from -2 to 300

# Print the box plot
print(coverage_boxplot)
...
# GPW: Population product area (adjusted for resolution)
camps$gpw_area <- camps$gpw * 1e6  # Since GPW is per km², multiply by 1e6 to convert to m²

# GHS-POP: Population product area (adjusted for 100m resolution)
camps$ghsl_area <- camps$ghsl * 100  # Multiply by 100 to adjust for the 100m resolution, resulting in area in m²

# WorldPop: Population product area (adjusted for 100m resolution)
camps$wp_c_area <- camps$wp_c * 100  # Multiply by 100 for 100m resolution
camps$wp_uc_area <- camps$wp_uc * 100  # Multiply by 100 for 100m resolution

# Step 3: Calculate coverage percentages
camps$gpw_percentage <- (camps$gpw_area / camps$cl_Shp_a) * 100
camps$ghsl_percentage <- (camps$ghsl_area / camps$cl_Shp_a) * 100
camps$wp_c_percentage <- (camps$wp_c_area / camps$cl_Shp_a) * 100
camps$wp_uc_percentage <- (camps$wp_uc_area / camps$cl_Shp_a) * 100
...
# Ensure Dataset is a factor with the correct order
coverage_data$Dataset <- factor(coverage_data$Dataset, levels = c("GPW-v4", "GHS-POP", "WorldPop_unconstrained", "WorldPop_constrained"))

# Step 5: Visualization - Box plot
coverage_boxplot <- ggplot(coverage_data, aes(x = Dataset, y = Coverage_Percentage, fill = Site_Status)) +
  geom_boxplot(alpha = 0.7, outlier.size = 2, outlier.colour = "red") +  # Box plot with outliers
  stat_summary(fun = median, geom = "point", shape = 23, size = 3, color = "blue", fill = "yellow") +  # Median as point
  stat_summary(fun = median, geom = "text", 
               aes(label = round(..y.., 1), group = Site_Status), 
               position = position_dodge(width = 0.75),  # Use position_dodge to separate the text labels
               vjust = -0.5, size = 3, color = "black") +  # Adjusted median label position
  scale_fill_manual(values = c("Formal" = "#76c893", "Informal" = "#f4a582")) +  # Custom colors for site status
  labs(
    title = "Population Product Coverage (%) of Refugee Settlement Boundary Area",
    x = "Dataset",
...
```{r}
# Save the plot as an image file
ggsave("D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/coverage_boxplot.png", plot = coverage_boxplot, width = 8, height = 6, dpi = 300)
```
#violin 
```{r}
library(sf)
...
# Ensure Site_Status is a factor with all possible levels
coverage_data$Site_Status <- factor(coverage_data$Site_Status, levels = c("Formal", "Informal"))

# Step 5: Visualization - Violin plot
coverage_violinplot <- ggplot(coverage_data, aes(x = Dataset, y = Coverage_Percentage, fill = Site_Status)) +
  geom_violin(alpha = 0.7, trim = TRUE) +  # Violin plot with trimming
  stat_summary(fun = median, geom = "text", 
               aes(label = round(..y.., 1), group = Site_Status), 
               position = position_dodge(width = 0.75),  # Use position_dodge to separate the text labels
               vjust = -0.5, size = 3, color = "black") +  # Adjusted median label position
...
# Ensure Site_Status is a factor with all possible levels (Formal, Informal)
coverage_data$Site_Status <- factor(coverage_data$Site_Status, levels = c("Formal", "Informal"))

# Debug step: Check the levels of 'Site_Status' for each dataset
table(coverage_data$Dataset, coverage_data$Site_Status)
...
# Step 4: Prepare data for visualization (only using GPW dataset)
coverage_data <- data.frame(
  Dataset = rep("GPW-v4", each = nrow(camps)),
  Coverage_Percentage = camps$gpw_percentage,
  Site_Status = camps$Sit_Stt  # Formal/Informal site status
)

# Step 5: Visualization - Box plot for only GPW with y-axis extended to max value
gpw_boxplot <- ggplot(coverage_data, aes(x = Dataset, y = Coverage_Percentage, fill = Site_Status)) +
  geom_boxplot(alpha = 0.7, outlier.size = 2, outlier.colour = "red") +  # Box plot with outliers
  stat_summary(fun = median, geom = "point", shape = 23, size = 3, color = "blue", fill = "yellow") +  # Median as point
  stat_summary(fun = median, geom = "text", 
               aes(label = round(..y.., 1), group = Site_Status), 
               position = position_dodge(width = 0.75),  # Separate the text labels for each Site_Status
               vjust = -0.5, size = 3, color = "black") +  # Adjusted median label position
  scale_fill_manual(values = c("Formal" = "#76c893", "Informal" = "#f4a582")) +  # Custom colors for site status
  labs(
    title = "GPW Population Product Coverage (%) of Refugee Settlement Boundary Area",
    x = "Dataset",
    y = "Coverage Percentage (%)",
    fill = "Site Status"
  ) +
  ...
  coord_cartesian(ylim = c(0, max(camps$gpw_percentage)))  # Extend y-axis to max GPW value (no log scale)

# Print the box plot for GPW
print(gpw_boxplot)
...
```{r}
# Save the plot as an image file
ggsave("D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/gpw_boxplot.png", plot = gpw_boxplot, width = 8, height = 6, dpi = 300)
```
#building area summary
```{r}
summarized_data <- camps %>%
  summarise(Google = sum(g_b_a, na.rm = TRUE),
            OSM = sum(osm_b_a, na.rm = TRUE),
            Microsoft = sum(ms_b_a, na.rm = TRUE),
            VIDA = sum(vida_b_a, na.rm = TRUE))
print(summarized_data)

summarized_data <- data.frame(
  source = c("Google", "OSM", "Microsoft", "VIDA"),
  building_count = c(3989102,1276338, 2602683, 4425513)
)
...
bar_plot <- ggplot(summarized_data, aes(x = source, y = building_count, fill = source)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = building_count), vjust = -0.5) +  # Add text labels above the bars
  labs(x = "Data Source", y = "Building Count", title = "Total Building Area by Data Source") +
  scale_fill_manual(values = c("Google" = "skyblue", "OSM" = "pink", "Microsoft" = "lightgreen", "VIDA" = "coral")) +
  theme_minimal() +
  theme(legend.position = "none")
...
print(bar_plot)
```
#correlation of building count and areas
```{r}
# Summarizing building areas and counts
summarized_data <- camps %>%
  summarise(
    Google_area = sum(g_b_a, na.rm = TRUE),
    Google_count = sum(g_b_c, na.rm = TRUE),
    OSM_area = sum(osm_b_a, na.rm = TRUE),
    OSM_count = sum(osm_b_c, na.rm = TRUE),
    Microsoft_area = sum(ms_b_a, na.rm = TRUE),
    Microsoft_count = sum(ms_b_c, na.rm = TRUE),
    VIDA_area = sum(vida_b_a, na.rm = TRUE),
    VIDA_count = sum(vida_b_c, na.rm = TRUE)
  )

# Creating a data frame with the summarized values
correlation_data <- data.frame(
  source = c("Google", "OSM", "Microsoft", "VIDA"),
  building_area = c(
    summarized_data$Google_area,
    summarized_data$OSM_area,
    summarized_data$Microsoft_area,
    summarized_data$VIDA_area
  ),
  building_count = c(
    summarized_data$Google_count,
    summarized_data$OSM_count,
    summarized_data$Microsoft_count,
    summarized_data$VIDA_count
  )
)

# Calculating correlation
correlation <- cor(correlation_data$building_area, correlation_data$building_count, use = "complete.obs")
print(paste("Correlation between building area and building count:", round(correlation, 2)))

# Visualizing the relationship
scatter_plot <- ggplot(correlation_data, aes(x = building_area, y = building_count, color = source)) +
  geom_point(size = 4) +
  geom_smooth(method = "lm", se = FALSE, color = "black", linetype = "dashed") +  # Linear regression line
  labs(
    x = "Total Building Area",
    y = "Building Count",
    title = "Relationship Between Building Area and Building Count",
...
```{r}
# Scatter plot with separate lm lines for each dataset
scatter_plot <- ggplot(correlation_data, aes(x = building_area, y = building_count, color = source)) +
  geom_point(size = 4) +
  geom_smooth(method = "lm", se = FALSE, aes(group = source)) +  # Separate lm lines for each source
  labs(
    x = "Total Building Area",
    y = "Building Count",
    title = "Relationship Between Building Area and Building Count",
    subtitle = "Separate Linear Regression Lines for Each Dataset"
  ) +
  theme_minimal() +
...
```{r}
# Reshape the data for long format to make it easier to facet by dataset
long_data <- camps %>%
  select(camp_id, g_b_a, g_b_c, osm_b_a, osm_b_c, ms_b_a, ms_b_c, vida_b_a, vida_b_c) %>%
  pivot_longer(
    cols = -camp_id,
...
# Plot scatterplot with geom_smooth for each dataset
scatter_plot <- ggplot(long_data, aes(x = a, y = c, color = source)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    x = "Building Area",
    y = "Building Count",
    title = "Building Count vs Building Area for Each Dataset",
    subtitle = "Linear Regression Lines for Each Dataset"
  ) +
  facet_wrap(~source) +
  theme_minimal() +
  theme(legend.position = "none")  # Legend is not necessary as datasets are separated in facets
...
# Reshape the data to long format for easier plotting
data_long <- camps_df %>%
  dplyr::select(Shlt_ty, g_b_a, ms_b_a, vida_b_a, osm_b_a, Site_Ar) %>%
  pivot_longer(cols = c(g_b_a, ms_b_a, vida_b_a, osm_b_a, Site_Ar), 
               names_to = "Variable", 
               values_to = "Value")

# Aggregate the data to get summed values by Shelter Type and Variable
data_aggregated <- data_long %>%
  group_by(Shlt_ty, Variable) %>%
  summarize(Aggregated_Value = sum(Value, na.rm = TRUE))
print(data_aggregated)
# Plotting a grouped bar plot with aggregated values on the bars
plot <- ggplot(data_aggregated, aes(x = Shlt_ty, y = Aggregated_Value, fill = Variable)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = Aggregated_Value), 
            position = position_dodge(width = 0.8), 
            vjust = -0.2, size = 2) +  # Adjust 'vjust' and 'size' for text positioning
  labs(x = "Shelter Type", y = "Area", title = "Aggregated area of Buildings and Survey Households by Shelter Type and Source") +
  theme_minimal() +
  theme(legend.position = "top", axis.text.x = element_text(angle = 45, hjust = 1))
...
# Calculate the correlation matrix for survey population (N_INDVI) and building counts
correlation_data <- camps_data_nogeom[, c("N_INDVI","N_HH", "g_b_c", "ms_b_c", "vida_b_c", "osm_b_c")]
...
# Initialize a list to store correlation results for each shelter type
shelter_correlation_results <- list()

# Unique shelter types
shelter_types <- unique(camps_data_nogeom$Shlt_ty)

# Loop through each shelter type
for (shelter in shelter_types) {
  # Filter data for the current shelter type
  shelter_data <- camps_data_nogeom[camps_data_nogeom$Shlt_ty == shelter, 
                                    c("N_INDVI", "N_HH", "g_b_c", "ms_b_c", "vida_b_c", "osm_b_c")]
  
  # Calculate correlations of N_HH and N_INDVI with the datasets
  correlation_matrix <- cor(shelter_data[, c("N_INDVI", "N_HH")], 
                            shelter_data[, c("g_b_c", "ms_b_c", "vida_b_c", "osm_b_c")], 
                            use = "complete.obs", 
                            method = "pearson")
  
  # Melt the correlation matrix for plotting
  correlation_melt <- melt(correlation_matrix, varnames = c("Survey_Variable", "Building_Dataset"))
  
  # Add shelter type information
  correlation_melt$Shelter_Type <- shelter
  
  # Store the results
  shelter_correlation_results[[shelter]] <- correlation_melt
}

# Combine all results into a single data frame
combined_correlation_results <- do.call(rbind, shelter_correlation_results)

# Create a heatmap for each shelter type
heatmap_plots <- list()
for (shelter in shelter_types) {
  # Filter data for current shelter type
  shelter_correlation_data <- combined_correlation_results[combined_correlation_results$Shelter_Type == shelter, ]
  
  # Create the heatmap
  heatmap_plot <- ggplot(data = shelter_correlation_data, aes(x = Building_Dataset, y = Survey_Variable, fill = value)) +
    geom_tile(color = "white") +
    ...
    labs(x = "Building Dataset", y = "Survey Variable", 
         title = paste("Correlation Heatmap for Shelter Type:", shelter))
  
  # Store the plot
  heatmap_plots[[shelter]] <- heatmap_plot
}

# Print all heatmaps
for (shelter in shelter_types) {
  print(heatmap_plots[[shelter]])
}


# Print the combined table of correlations
print(combined_correlation_results)

# Save the correlation table as a CSV file
write.csv(combined_correlation_results, D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots, "correlation_results.csv"), row.names = FALSE)
...
for (shelter in shelter_types) {
  # Filter data for the current shelter type
  shelter_data <- camps_data_nogeom[camps_data_nogeom$Shlt_ty == shelter, 
                                    c("N_HH", "g_b_c", "ms_b_c", "vida_b_c", "osm_b_c")]
  
  # Calculate correlations of N_HH with the datasets
  correlation_matrix <- cor(shelter_data[, c("N_HH")], 
                            shelter_data[, c("g_b_c", "ms_b_c", "vida_b_c", "osm_b_c")], 
                            use = "complete.obs", 
                            method = "pearson")
...
# Create a single correlation plot for all shelters, focusing on N_HH correlation
correlation_plot <- ggplot(data = combined_correlation_results, aes(x = Building_Dataset, y = Shelter_Type, fill = value)) +
  geom_tile(color = "white") +
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0, 
                       limit = c(-1, 1), space = "Lab", 
                       name = "Pearson\nCorrelation") +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "Building Dataset", y = "Shelter Type", title = "Correlation of Survey Household Counts (N_HH) with Building counts for different shelter types")

# Print the correlation plot
print(correlation_plot)

# Print the combined correlation table (optional)
print(combined_correlation_results)
...
for (shelter in shelter_types) {
  ...
  # Replace the dataset names with more descriptive labels
  correlation_melt$Building_Dataset <- recode(correlation_melt$Building_Dataset,
                                              "g_b_c" = "Google",
                                              "ms_b_c" = "Microsoft",
                                              "vida_b_c" = "VIDA",
                                              "osm_b_c" = "OSM")
...
correlation_plot <- ggplot(data = combined_correlation_results, aes(x = Building_Dataset, y = Shelter_Type, fill = value)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "Building Dataset", y = "Shelter Type", title = "Correlation of Household Counts (N_HH) with Building Datasets")
...
#heat map save
# Save the plot as an image file
ggsave("D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/correlation_plot.png", plot = correlation_plot, width = 8, height = 6, dpi = 300)
```
#normalized building count
```{r}
bu
...
```{r}
# Step 1: Calculate coverage areas for each dataset (if not already done)
camps$google_area <- camps$g_b_a / camps$Site_Ar * 100  # Google building area coverage percentage
camps$osm_area <- camps$osm_b_a / camps$Site_Ar * 100      # OSM building area coverage percentage
camps$ms_area <- camps$ms_b_a / camps$Site_Ar * 100        # Microsoft building area coverage percentage
camps$vida_area <- camps$vida_b_a / camps$Site_Ar * 100    # VIDA building area coverage percentage

# Step 2: Prepare data for visualization with shelter type included
coverage_data <- data.frame(
  Dataset = rep(c("Google", "OSM", "Microsoft", "VIDA"), each = nrow(camps)),
  Coverage_Percentage = c(camps$google_area, camps$osm_area, camps$ms_area, camps$vida_area),
  Site_Status = camps$Sit_Stt,  # Formal/Informal site status
  Shelter_Type = rep(camps$Shlt_ty, 4)  # Shelter type repeated for each dataset
)

# Ensure Dataset is a factor with the correct order
coverage_data$Dataset <- factor(coverage_data$Dataset, levels = c("Google", "Microsoft", "VIDA", "OSM"))

# Step 3: Visualization - Violin plot grouped by shelter type
coverage_violin_plot <- ggplot(coverage_data, aes(x = Dataset, y = Coverage_Percentage, fill = Shelter_Type)) +
  geom_violin(trim = FALSE, alpha = 0.7) +
  stat_summary(
    aes(color = Shelter_Type),
    fun = median, geom = "point", size = 3, position = position_dodge(width = 0.9)
  ) +
  stat_summary(
    aes(label = round(..y.., 1), color = Shelter_Type),
    fun = median, geom = "text", position = position_dodge(width = 0.9), vjust = -0.5, size = 3
  ) +
  scale_fill_brewer(palette = "Set3", name = "Shelter Type") +  # Use a palette for clear distinctions
  scale_color_manual(values = c("blue", "black", "red", "green")) +  # Adjust colors as needed
  labs(
    title = "Building Area Coverage (%) of Refugee Settlement Boundary Area by Shelter Type",
    x = "Dataset",
    y = "Coverage Percentage (%)",
    fill = "Shelter Type"
  ) +
  ...
  coord_cartesian(ylim = c(0, 100))  # Adjust y-axis range if needed
...
```
```{r}
# Automatically generate a palette that accommodates all shelter types
coverage_violin_plot <- ggplot(coverage_data, aes(x = Dataset, y = Coverage_Percentage, fill = Shelter_Type)) +
  geom_violin(trim = FALSE, alpha = 0.7) +
  stat_summary(
    aes(color = Shelter_Type),
  ...
  scale_fill_brewer(palette = "Set3", name = "Shelter Type") +  # Automatically adjusts for all categories
  labs(
    title = "Building Area Coverage (%) of Refugee Settlement Boundary Area by Shelter Type",
    x = "Dataset",
    y = "Coverage Percentage (%)",
...
```{r}
# Save the plot as an image file
ggsave("D:/EAGLE/Master_Thesis_Hannes/1/R/Lallu_thesis/Plots/coverage_violin_plot.png", plot = coverage_violin_plot, width = 15, height = 6, dpi = 300)
```
