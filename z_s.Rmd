...
```{r}
# Create the box plot with adjusted y-axis limits and outlier color
boxplot_ratio <- ggplot(difference_data, aes(x = Dataset, y = ratio)) +
  geom_boxplot(fill = "coral", color = "black", outlier.color = "lightgray") +
  stat_summary(fun = mean, geom = "point", shape = 3, size = 1, color = "blue") +
  geom_text(data = mean_ratio_values, aes(label = round(ratio, digits = 2)), vjust = -0.5, size = 2,color='blue') +
  labs(title = "Ratio of Difference to Survey Population for Different EO Population Data Products",
       x = "Dataset", y = "Ratio of Difference to Survey Population") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
summarized_data <- data.frame(
  ...
  building_count = c(170437, 26323, 67317, 185603)
)
...
# Create a box plot
boxplot_build_diff <- ggplot(difference_data_building, aes(x = Dataset_build, y = Building_Difference)) +
  geom_boxplot(fill = "skyblue", color = "black", outlier.alpha = 0.2, width = 2) + # Increase box width
  stat_summary(fun = mean, geom = "point", shape = 23, size = 2, color = "red") + # Adding mean points with larger size
  geom_text(data = mean_values, aes(label = round(Building_Difference, digits = 2)), vjust = -0.5, size = 3, color = "red") + # Adding larger text labels
  labs(title = "Difference between Surveyed Household and Building Count (Survey-Estimate) for Different Building Footprint Data Products",
       x = "Dataset", y = "Building Count Difference") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
...
# Create a box plot for the ratios
boxplot_ratio_build <- ggplot(difference_data_building, aes(x = Dataset_build, y = ratio)) +
  geom_boxplot(fill = "pink", color = "black") + # Adjust outlier transparency
  stat_summary(fun = mean, geom = "point", shape = 23, size = 2, color = "red") + # Adding mean points with larger size
  geom_text(data = mean_ratio_values_build, aes(label = round(ratio, digits = 2)), vjust = -0.5, size = 3, color = "red") + # Adding larger text labels
  labs(title = "Ratio of Difference to Survey Household for Different Building Footprint Data Products",
       x = "Dataset", y = "Ratio of Difference to Survey Household number") +
...
# Define outlier symbol
outlier_symbol <- geom_point(shape = 4, color = "red", size = 2)

# Create a box plot for the ratios
boxplot_ratio_build <- ggplot(difference_data_building, aes(x = Dataset_build, y = ratio)) +
  geom_boxplot(fill = "pink", color = "black", outlier.shape = NA) + # Hide outliers in the boxplot
  geom_point(position = position_jitter(width = 0.2), color = "pink", alpha = 0.3) + # Lighten outlier color
  stat_summary(fun = mean, geom = "point", shape = 2, size = 2, color = "blue") + # Changing mean point color to blue
  geom_text(data = mean_ratio_values_build, aes(label = round(ratio, digits = 2)), vjust = -0.5, size = 3, color = "blue") + # Changing mean text color to blue
  labs(title = "Ratio of Difference to Survey Household for Different Building Footprint Data Products",
       x = "Dataset", y = "Ratio of Difference to Survey Household number") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        ...
        plot.margin = margin(1, 1, 1, 1, "cm")) + # Add more margin space around the plot
  coord_cartesian(ylim = c(-2, 2)) + # Adjusting ylim to focus on the main distribution area
  geom_errorbar(aes(ymin = -2, ymax = -2), width = 0.2, color = "red", size = 1) + # Adding error bars at ylim for outliers
  geom_errorbar(aes(ymin = 2, ymax = 2), width = 0.2, color = "red", size = 1) + # Adding error bars at ylim for outliers
  guides(fill=FALSE) # Hiding legend for fill color
