...
# Summary statistics
survey_stats <- c(Min = 934.1, Q1 = 6741.2, Median = 26100.6, Mean = 114271.1, Q3 = 104571.3, Max = 1606318.8)
calculated_stats <- c(Min = 913.1, Q1 = 7747.7, Median = 27405.4, Mean = 122524.5, Q3 = 132531.1, Max = 1654478.8)
...
```{r}
# Summary statistics
surv_sta <- c(Min = 26.0, Q1 = 327.5, Median = 1058.0, Mean = 4134.3, Q3 = 4575.5, Max = 43074.0)
...
With a variance of 55688371 and the summary statistics you provided earlier for the N_INDVI variable:
...
3rd Quartile: 4575.5
Maximum: 43074.0
We can infer the following about the data:

Spread: The data has a wide spread, as indicated by the large difference between the minimum and maximum values.

Central Tendency: The mean (4134.3) is much higher than the median (1058.0), suggesting that the data might be positively skewed.

Variability: The variance (55688371) is quite large, indicating that the data points are spread out over a wide range from the mean.

Outliers: The presence of outliers at the higher end of the distribution is suggested by the large difference between the 3rd quartile (4575.5) and the maximum (43074.0).

Overall, these statistics suggest that the data is positively skewed, with a few very high values driving up the mean and variance, while the majority of the data is clustered towards the lower end of the distribution.

Positively skewed data means that the distribution of the data is skewed towards the right side, or the tail of the distribution is longer on the right side. In other words, the majority of the data points are concentrated on the left side of the distribution, with a few extremely high values pulling the mean towards the right.

In a positively skewed distribution:

The mean is typically greater than the median.
The tail of the distribution extends towards the higher values.
Most of the data points are clustered towards the lower end of the distribution.
Outliers are more likely to be present on the higher end of the scale.
Positively skewed distributions are also sometimes referred to as right-skewed distributions.
...
difference_data_building <- data.frame(
  Dataset_build = rep(c("GOOGLE", "MICROSOFT", "OSM", "VIDA"), each = nrow(camps)),
  Building_Difference = c(as.numeric(camps$g_bc_df), as.numeric(camps$ms_bc_df),as.numeric(camps$vida_bc_df), as.numeric(camps$osm_bc_df)))
