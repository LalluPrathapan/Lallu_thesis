...
ghsl_surv_ls500 <- ggplot(subset(camps, as.numeric(N_INDVI) < 500), aes(y = ghsl, x = N_INDVI, color = Sit_Stt)) +
  ...
  theme_minimal() +
  scale_y_continuous(breaks = seq(0, 1000, by = 100))  # Adjust y-axis intervals
...
custom_colors <- c("#FF5733", "#33FF57", "#3366FF", "#FF33A8", "#FFFF33", "#33FFFF", "#FF3399", "#9933FF")
...
ggplot(filt_data_shltyls, aes(x = as.numeric(ghsl), y = as.numeric(N_INDVI), color = Shlt_ty)) +
...
plot1 <- ggplot(camps, aes(x = N_INDVI, y = N_HH, color = ghsl)) +
  geom_point() +
...
plot2 <- ggplot(camps, aes(x = N_INDVI, y = ghsl, color = ghsl)) +
  geom_point() +
...
plot3 <- ggplot(camps, aes(x = N_HH, y = ghsl, color = ghsl)) +
  geom_point() +
