...
#(settlement class) spontaneity
# plot the calc vs survey for 
...
#correlation between houshold and population(both survey and estimated WP_C)
...
# Calculate absolute difference
absolute_difference <- abs(camps$wp_c - camps$N_INDVI)

# Calculate percentage difference
percentage_difference <- abs((camps$wp_c - camps$N_INDVI) / camps$N_INDVI) * 100

# Calculate root mean square error (RMSE)
rmse <- sqrt(mean((camps$wp_c - camps$N_INDVI)^2))

# Calculate mean absolute percentage error (MAPE)
mape <- mean(percentage_difference, na.rm = TRUE)

# Calculate absolute error
absolute_error <- mean(abs(as.numeric(camps$wp_c) - as.numeric(camps$N_INDVI)))

# Print the result
print(absolute_error)

# Calculate percentage error
percentage_error <- mean(abs(as.numeric(camps$wp_c) - as.numeric(camps$N_INDVI)) / as.numeric(camps$N_INDVI)) * 100
...
# Subtract N_INDVID from ras_name
camps$gpw_diff <- camps$N_INDVI - camps$gpw
...
#correlation between houshold and population(both survey and estimated GPW)
...
# Calculate absolute difference
absolute_difference <- abs(camps$gpw - camps$N_INDVI)

# Calculate percentage difference
percentage_difference <- abs((camps$gpw - camps$N_INDVI) / camps$N_INDVI) * 100

# Calculate root mean square error (RMSE)
rmse <- sqrt(mean((camps$gpw - camps$N_INDVI)^2))

# Calculate mean absolute percentage error (MAPE)
mape <- mean(percentage_difference, na.rm = TRUE)

# Calculate absolute error
absolute_error <- mean(abs(as.numeric(camps$gpw) - as.numeric(camps$N_INDVI)))

# Print the result
print(absolute_error)

# Calculate percentage error
percentage_error <- mean(abs(as.numeric(camps$gpw) - as.numeric(camps$N_INDVI)) / as.numeric(camps$N_INDVI)) * 100
...
camps$N_INDVI <- as.numeric(camps$N_INDVI)
camps$ghsl <- as.numeric(camps$ghsl)
camps$wp_con <- as.numeric(camps$wp_con)
camps$wp_uncon <- as.numeric(camps$wp_uncon)
camps$gpw <- as.numeric(camps$gpw)
camps$gpw_UN<-as.numeric(camps$gpw_UN)

# Check the data types again
str(camps[, c("N_INDVI", "ghsl", "wp_con", "wp_uncon", "gpw","gpw_UN")])

# Check for missing values
summary(camps[, c("N_INDVI", "ghsl", "wp_con", "wp_uncon", "gpw","gpw_UN")])

# Calculate correlation coefficients
corr_ghsl <- cor(camps$N_INDVI,camps$ghsl)
corr_wp_con<-cor(camps$N_INDVI,camps$wp_con)
corr_wp_uncon<-cor(camps$N_INDVI,camps$wp_uncon)
...
cor_matrix <- matrix(c(corr_ghsl, corr_wp_con, corr_wp_uncon, corr_gpw,gpw_UN),
                     nrow = 1, byrow = TRUE,
                     dimnames = list(c("N_INDVI"), c("ghsl", "wp_con", "wp_uncon", "gpw","gpw_UN")))
...
numeric_cols <- c("N_INDVI", "ghsl", "wp_con", "wp_uncon", "gpw","gpw_UN")
