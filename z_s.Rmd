...
ggplot(build_stats_inc, aes(x = Site_Ar, y = cal_Shape_Area)) +
  geom_point(aes(color = "cal_Shape_Area"), size = 3) +
  geom_point(aes(color = "Site Area"), size = 1) +
  scale_color_manual(values = c("cal_Shape_Area" = "blue", "Site Area" = "red")) +
...
#things done to retain the geometry

```{r}
# Retain the original geometry
geom_var_dup<-build_stats_inc
clipped_data <- st_sf(geometry = geom_var_dup$geometry, st_drop_geometry(clipped_data))
```
...
```{r}
# Extract the unique identifiers for both datasets
unique_ids_clipped <- clipped_data$Name
unique_ids_build_stats <- build_stats_inc$Name

# Find the common unique identifiers
common_ids <- intersect(unique_ids_clipped, unique_ids_build_stats)

# Filter the datasets based on the common unique identifiers
clipped_data <- clipped_data[clipped_data$common_id %in% common_ids, ]
build_stats_inc <- build_stats_inc[build_stats_inc$common_id %in% common_ids, ]

# Retain the original geometry
clipped_data <- st_set_geometry(clipped_data, build_stats_inc$geometry)

# Print the clipped_data to inspect the results
str(clipped_data)
